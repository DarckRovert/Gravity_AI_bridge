...
def _ddg_search(query: str) -> List[Dict[str, str]]:
        ...
        return [
            {
                "title": "Error de Red",
                "url": "",
                "snippet": f"No se pudo consultar DuckDuckGo: {str(e)}",
            }
        ]
    ...
    urls: List[str] = re.findall(
        r'class="result__url"[^>]*>\s*(.*?)\s*</a>', html, re.DOTALL
    )
...
class WebSearch(Tool):
    ...
    name: str = "web_search"
    description: str = (
        "Busca información actualizada en la web y la retorna estructurada."
    )
    ...
    def execute(self, query: str, **kwargs: Any) -> ToolResult:
        ...
        if not results:
            return ToolResult(
                success=False,
                stderr="No se pudieron recuperar resultados de búsqueda web.",
            )
