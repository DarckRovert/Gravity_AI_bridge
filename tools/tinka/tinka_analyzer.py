...
class TinkaAnalyzer:
    ...
    def get_number_frequencies(self):
        ...
        for draw in draws:
            all_numbers.extend([draw['b1'], draw['b2'], draw['b3'], draw['b4'], draw['b5'], draw['b6']])
            
        counter = Counter(all_numbers)
        # Ordenar por frecuencia descendente
        sorted_freq = dict(sorted(counter.items(), key=lambda item: item[1], reverse=True))
        return sorted_freq
    ...
    def get_droughts(self, current_draw_num):
        ...
        max_num = max([n for draw in draws for n in (draw['b1'], draw['b2'], draw['b3'], draw['b4'], draw['b5'], draw['b6'])] + [50])
        ...
        for draw in draws:
            ...
            numbers = [draw['b1'], draw['b2'], draw['b3'], draw['b4'], draw['b5'], draw['b6']]
            for n in numbers:
    ...
    def get_markov_transitions(self, top_n=5):
        ...
        for i in range(len(draws) - 1):
            ...
            curr_nums = [current_draw['b1'], current_draw['b2'], current_draw['b3'], current_draw['b4'], current_draw['b5'], current_draw['b6']]
            next_nums = [next_draw['b1'], next_draw['b2'], next_draw['b3'], next_draw['b4'], next_draw['b5'], next_draw['b6']]
        ...
        latest_draw = draws[-1]
        latest_nums = [latest_draw['b1'], latest_draw['b2'], latest_draw['b3'], latest_draw['b4'], latest_draw['b5'], latest_draw['b6']]
        ...
        for num in latest_nums:
            ...
            for nxt, count in top_transitions:
                prob = round((count / total_transitions) * 100, 2) if total_transitions > 0 else 0
