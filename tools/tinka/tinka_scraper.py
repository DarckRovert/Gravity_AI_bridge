...
class TinkaScraper:
    ...
    def fetch_latest_results(self):
        ...
        try:
            ...
            urls = re.findall(r'<loc>(https://www.tinkaresultados.com/sorteos-historicos/jugada-[^<]+)</loc>', r.text)
            if not urls:
                return False
                
            latest_url = urls[0] # El sitemap usualmente pone el último primero (o podemos buscar el ID más alto)
            ...
            meta_match = re.search(r'jugada-(\d+)-del-(\d{1,2}-\d{1,2}-\d{4})', best_url)
            if meta_match:
            ...
            self.db.insert_draw(draw_num, draw_date, numeros[0], numeros[1], numeros[2], numeros[3], numeros[4], numeros[5], boliyapa)
        ...
        try:
            ...
            def fetch_draw(url):
                try:
                    ...
                    meta_match = re.search(r'jugada-(\d+)-del-(\d{1,2}-\d{1,2}-\d{4})', url)
                    if meta_match:
                        draw_num = int(meta_match.group(1))
            ...
            if len(valid_draws) > 0:
                ...
                logger.info(f"Crawling masivo finalizado. {inserted} sorteos reales guardados.")
                return inserted
            else:
                logger.warning("No se extrajeron sorteos válidos. Conservando la base de datos actual.")
                return 0
