    ...
    with _settings_lock:
        if os.path.exists(SETTINGS_FILE):
            try:
                ...
                console.print("[bold red]⚠ Advertencia:[/] _settings.json está corrompido o inaccesible. Cargando valores por defecto.")
...
def save_settings(data: Dict[str, Any]) -> None:
    ...
    with _settings_lock:
        ...
        try:
            ...
            console.print(f"[bold red]⚠ Error al guardar configuraciones de forma concurrente:[/] {str(e)}")
...
def draw_dashboard(results: List[Any]) -> None:
    ...
    for r in results:
        ...
        tier: str = "☁ Cloud" if getattr(r, "category", "local") == "cloud" else "💻 Local"
...
def main() -> None:
    ...
    console.print(Align.center(Panel(
        Text("SISTEMA DE DIAGNÓSTICO V13.0 PRO", justify="center", style="bold bright_white"), 
    ...
    if healthy_count == 0:
        ...
        console.print("[yellow]Por favor, inicia un motor local o usa key_manager.py para añadir claves e inténtalo de nuevo.[/]")
        sys.exit(1)
    ...
    current_prov: Optional[Any] = next(
        (s for s in scans if s.protocol == settings.get("provider_protocol") and s.is_healthy), 
        None
    )
    if not current_prov:
