...
def load_settings():
    ...
    return {"last_model": "deepseek-r1:8b", "provider": "ollama", "provider_protocol": "ollama", "api_url": "http://localhost:11434"}
...
def draw_dashboard(results):
    table = Table(title="📡 MAPA DE PROVEEDORES (LOCAL + CLOUD)", box=box.HEAVY_EDGE, border_style="cyan", title_style="bold bright_white")
...
def prompt_menu(results):
    ...
    for i, r in enumerate(healthy, 1):
        ...
        tag = "[bold yellow](En Memoria)[/]" if r.active_model else ""
        tier = "[Cloud]" if getattr(r, "category", "local") == "cloud" else "[Local]"
...
def main():
    ...
    console.print(Align.center(Panel(Text("SISTEMA DE DIAGNÓSTICO V12.2 PRO", justify="center", style="bold bright_white"), style="on bright_black", box=box.HEAVY_EDGE, padding=(0,2))))
    ...
    if healthy_count == 0:
        ...
        console.print("\n[bold red]✖ ERROR CRÍTICO:[/] No se detectó ningún proveedor de IA (Ollama, LM Studio, u OpenAI/Anthropic/DeepSeek Keys guardadas) en ejecución.")
        console.print("[yellow]Por favor, inicia un motor local o usa key_manager.py para añadir claves Cloud e inténtalo de nuevo.[/]")
        sys.exit(1)
        
    settings = load_settings()
    
    # Check current
    current_prov = next((s for s in scans if s.protocol == settings.get("provider_protocol") and s.is_healthy), None)
    if not current_prov:
        current_prov = next((s for s in scans if s.name == settings.get("provider") and s.is_healthy), None)
