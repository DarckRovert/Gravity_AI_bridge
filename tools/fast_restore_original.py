...
def main():
    for b in [1, 2, 3]:
        ...
        for cap_path in orig_caps:
            ...
            logger.info(f"  {cap_name} restaurado (con {len(images)} imágenes inyectadas).")

        # Ensamblar libro y HTML
        title = f"Cenizas del Leviatán Libro {b} (Original Ilustrado)"
        ref_caps = _detect_caps(ref_dir)
        assembled = _assemble_book(ref_dir, title, sorted(ref_caps, key=lambda p: int(re.search(r'cap_(\d+)', p).group(1))))
        
        book_md_path = os.path.join(ref_dir, f"Cenizas_del_Leviatan_Libro_{b}_refinado.md")
