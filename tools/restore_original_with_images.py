...
DEVUELVE ÚNICAMENTE EL PROMPT EN INGLÉS, sin comillas ni introducciones."""
    ...
    return (
        resp.strip()
        if resp
        else "Cinematic cyberpunk scene, neo-noir, dark atmosphere, 8k"
    )


def process_chapter(
    original_cap_path: str, out_dir: str, cap_name: str, book_name: str
):
    ...
    if len(paragraphs) < 5:
        ...
        paragraphs_to_illustrate = [
            max(1, len(paragraphs) // 4),
            min(len(paragraphs) - 2, (len(paragraphs) // 4) * 3),
        ]
    ...
    for i, p in enumerate(paragraphs):
        if i in paragraphs_to_illustrate and len(p.strip()) > 50:
            logger.info(
                f"    Generando prompt de imagen para {cap_name} (sección {i})..."
            )
...
def main():
    ...
    for b in books:
        ...
        book_md_path = os.path.join(
            out_dir, f"Cenizas_del_Leviatan_Libro_{b}_refinado.md"
        )
