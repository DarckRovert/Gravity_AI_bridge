...
class GitTool(Tool):
    ...
    def execute(self, operation: str = "status", **kwargs: Any) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(success=False, stderr=f"Error inesperado ejecutando Git: {str(e)}")
...
class FileOpsTool(Tool):
    ...
    """
    name: str = "file_ops"
    description: str = "Operaciones de archivo seguras: respaldos, diferencias y aplicación de parches."
    ...
    def backup(self, path: str) -> ToolResult:
        ...
        if not path or not os.path.exists(path):
            return ToolResult(success=False, stderr=f"El archivo especificado para respaldo no existe: '{path}'")
        bak: str = path + ".gravity.bak"
        try:
            shutil.copy2(path, bak)
            return ToolResult(success=True, stdout=f"Copia de seguridad creada con éxito en: {bak}")
        except IOError as e:
            return ToolResult(success=False, stderr=f"Error de E/S al crear copia de seguridad: {str(e)}")
        except Exception as e:
            return ToolResult(success=False, stderr=f"Error inesperado al crear copia de seguridad: {str(e)}")
    ...
    def diff(self, original_path: str, new_content: str) -> ToolResult:
        ...
        if not original_path or not os.path.exists(original_path):
            return ToolResult(success=False, stderr=f"El archivo original para comparar no existe: '{original_path}'")
        try:
            ...
            diff_lines: List[str] = list(difflib.unified_diff(orig, new, fromfile=original_path, tofile="[nuevo]"))
            return ToolResult(success=True, stdout="".join(diff_lines[:200]), language="diff")
        except IOError as e:
            return ToolResult(success=False, stderr=f"Error de E/S al leer '{original_path}': {str(e)}")
        except Exception as e:
            return ToolResult(success=False, stderr=f"Error inesperado al generar diff: {str(e)}")
        ...
        if match:
            ...
            try:
                ...
                with open(path, "w", encoding="utf-8") as f:
                    f.write(code)
                return ToolResult(success=True, stdout=f"✓ Parche aplicado exitosamente en {path}")
            except IOError as e:
                return ToolResult(success=False, stderr=f"Fallo de escritura en archivo: {str(e)}")
            except Exception as e:
                return ToolResult(success=False, stderr=f"Error inesperado aplicando parche: {str(e)}")
    ...
    def execute(self, action: str = "diff", **kwargs: Any) -> ToolResult:
        ...
        return ToolResult(success=False, stderr=f"Acción de sistema '{action}' no es reconocida.")
