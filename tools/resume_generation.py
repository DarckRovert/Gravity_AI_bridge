...
def resume_book(title):
    ...
    previous_summary = ""
    if last_chap > 0:
        with open(os.path.join(book_dir, f"cap_{last_chap}.md"), "r", encoding="utf-8") as f:
            prev_text = f.read()
            previous_summary = author._summarize_chapter(prev_text)
            
    full_outline_text = "\n".join([f"Cap {c.get('numero')}: {c.get('resumen_eventos')}" for c in outline])
    source_text = author._extract_text_from_google_docs("https://docs.google.com/document/d/17kUPrfLZAJc8F5Nj54co4BUO_GZMspo3CNZXnRBFop0/export?format=txt")
    ...
    for chap in outline:
        ...
        chapter_text = author._write_chapter(chap, base_context, full_outline_text, previous_summary, source_text)
