...
def generate_epub(target_dir: str) -> str:
    ...
    book_title = title_match.group(1).strip() if title_match else dir_name.replace("_", " ")
    ...
    for i, sec_text in enumerate(sections):
        ...
        html_content = markdown.markdown(sec_text, extensions=["tables"], output_format="xhtml")
                    ...
                    if not book.get_item_with_id(img_src):
                        try:
                            with open(img_path, "rb") as f:
                                ...
                                img_item = epub.EpubImage(uid=img_src, file_name=img_src, media_type=mime, content=f.read())
                                book.add_item(img_item)
                                logger.info(f"    Imagen secundaria empaquetada: {img_src}")
                        except Exception as e:
                            logger.error(f"    Error empaquetando imagen {img_src}: {e}")
