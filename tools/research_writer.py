...
class GravityResearchAuthor:
    ...
    def _generate_book_cover(self, book_dir: str, title: str, synopsis: str) -> Optional[str]:
        ...
        logger.info(f"Fase Visual: generando portada con ImageRouter (prompt: {image_prompt[:50]}...)")
    ...
    def _generate_outline(self, synopsis: str, num_chapters: int, user_prompt: str) -> list:
        if num_chapters > 0:
        ...
        if isinstance(data, dict):
            ...
            logger.error("Fallo irrecuperable al generar escaleta. Se usará una genérica.")
            fallback_len = num_chapters if num_chapters > 0 else 5
            return [{"numero": i, "titulo": f"Capítulo {i}", "resumen_eventos": "Argumentación teórica."} for i in range(1, fallback_len + 1)]

    def _do_web_search_for_chapter(self, chap_title: str, chap_events: str, user_prompt: str) -> str:
        logger.info(f"Fase 2.5: Formulando consultas de investigaci\u00f3n para: '{chap_title}'...")
                    ...
                    for url in urls[:2]:
                        if url not in seen_urls:
                            ...
                            if not page_text.startswith("[fetch"):
                                all_results.append(f"  [Contenido de {url[:60]}...]\n  {page_text[:1500]}")
                else:
        ...
        combined = "\n\n".join(all_results)
        logger.info(f"  Investigaci\u00f3n completada: {len(queries)} queries, {len(seen_urls)} p\u00e1ginas extra\u00eddas.")
        return combined[:8000]  # cap para no saturar el contexto del LLM


    def _write_chapter(self, chapter_data: dict, synopsis: str, full_outline_text: str, accumulated_history: str, search_results: str, user_prompt: str) -> str:
...
"""
        ...
        for i in range(max_continuations):
            ...
            if stripped_end and not stripped_end[-1] in ".?!\"'*:":
                logger.warning(f"Capítulo {chap_num} posiblemente truncado (parte {i+1}). Pidiendo continuación...")
                messages.append({"role": "assistant", "content": response})
                messages.append({"role": "user", "content": "La respuesta parece haberse cortado. Continúa exactamente desde donde te quedaste, sin repetir palabras."})
                time.sleep(2)
            else:
                break
        ...
        if cleaned != full_chapter_text.strip():
            logger.info(f"Capítulo {chap_num}: LaTeX residual limpiado por latex_cleaner.")
        return cleaned
    ...
    def write_research_book(self, prompt: str, title: str = "Ensayo", num_chapters: int = 5, max_free_chapters: int = 20):
        ...
        if os.path.exists(progress_file) and os.path.exists(os.path.join(book_dir, "2_escaleta.json")):
            with open(progress_file, "r", encoding="utf-8") as f:
                data = json.load(f)
                start_chapter = data.get("ultimo_capitulo", 0) + 1
            with open(os.path.join(book_dir, "2_escaleta.json"), "r", encoding="utf-8") as f:
                outline = json.load(f)
            with open(os.path.join(book_dir, "1_sinopsis.md"), "r", encoding="utf-8") as f:
                base_context = f.read()
        ...
        if num_chapters <= 0:
            if len(outline) > max_free_chapters:
                logger.warning(f"LLM generó {len(outline)} capítulos en modo libre. Limitando a {max_free_chapters} (max_free_chapters).")
...
Propón que la libertad humana depende de la capacidad del individuo para recuperar el control sobre su propia 'tecnología mental' antes de que sea utilizada como combustible por el sistema."""
    ...
    author.write_research_book(prompt=prompt, title="La Física del Poder", num_chapters=5)
