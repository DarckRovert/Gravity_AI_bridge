...
async def run_gradio(
    ...
    ws_url: str = (
        fooocus_url.replace("http://", "ws://").replace("https://", "wss://")
        + "/queue/join"
    )
    ...
    try:
        ...
        print(
            json.dumps({"success": False, "error": f"Error conectando a Fooocus: {e}"})
        )
        return
    ...
    if not get_task_fn or not gen_clicked_fn:
        print(
            json.dumps(
                {"success": False, "error": "generadores web no encontrados en config"}
            )
        )
        return
    ...
    for cid in get_task_fn["inputs"]:
        ...
        if cid == cid_map.get("prompt"):
            ...
            val = (
                negative_prompt
                if negative_prompt
                else "low quality, bad anatomy, text, watermark, deformed"
            )
            ...
            if cid in comp_choices:
                for choice in comp_choices[cid]:
                    ...
                    if isinstance(choice_val, str) and choice_val.startswith(
                        base_aspect
                    ):
                        val = choice_val
                        break
        ...
        if val is None and cid in comp_choices and len(comp_choices[cid]) > 0:
            ...
            val = (
                first_choice[1]
                if isinstance(first_choice, list) and len(first_choice) > 1
                else first_choice
            )
    ...
    session_hash = "gravity_" + "".join(
        random.choices(string.ascii_lowercase + string.digits, k=10)
    )
    print(
        f"[NativeTrigger] WS State init. (session: {session_hash}, params: {len(args)})",
        file=sys.stderr,
    )
    ...
    try:
        ...
        print(
            f"[NativeTrigger] WS Generando imagen (fn_index: {gen_clicked_fn['index']})",
            file=sys.stderr,
        )
                ...
                if data["msg"] == "send_hash":
                    ...
                    for im in gallery:
                        if isinstance(im, dict) and "name" in im:
                            ...
                            isinstance(im, list)
                            and len(im) > 0
                            and isinstance(im[0], dict)
                            and "name" in im[0]
                        ):  # Gradio < 3.42 fallback
    ...
    if len(sys.argv) < 4:
        print(
            json.dumps(
                {
                    "success": False,
                    "error": "Faltan argumentos (prompt, performance, aspect_ratio)",
                }
            )
        )
        sys.exit(1)
