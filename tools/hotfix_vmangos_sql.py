...
def main() -> None:
    ...
    api_url: str = (
        "https://api.github.com/repos/vmangos/core/contents/sql/migrations?ref=development"
    )
    req: urllib.request.Request = urllib.request.Request(
        api_url, headers={"User-Agent": "GravityBridge-v10"}
    )
    ...
    for m_id in migrations:
        ...
        try:
            rq: urllib.request.Request = urllib.request.Request(
                target_url, headers={"User-Agent": "GravityBridge-v10"}
            )
            ...
            print(
                f"[ERROR] Excedido el límite de tiempo (30s) al aplicar la migración {m_id}."
            )
        except Exception as e:
