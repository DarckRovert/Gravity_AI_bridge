...
def _get_provider_status() -> str:
    ...
    try:
        ...
        for s in scans:
            ...
            lines.append(
                f"  - {s.name}: {status} | {len(s.models)} modelos | latencia {lat}"
            )
...
def _get_video_status() -> str:
    ...
    try:
        ...
        lines = [
            f"Video Studio: FFmpeg={'OK' if ffmpeg_ok else 'NO INSTALADO'} | {pending} en cola | {history_count} histórico"
        ]
        if current:
            lines.append(
        ...
        for job in data.get("history", [])[:3]:
            st = job.get("status", "?")
            lines.append(
                f"  ▸ Job #{job.get('id')}: {job.get('topic', '?')[:40]} | {st}"
            )
        return "\n".join(lines)
    except Exception as e:
...
def _get_cost_status() -> str:
    ...
    try:
        ...
        total_tok = int(session_tokens.get("input", 0)) + int(
            session_tokens.get("output", 0)
        )
        return (
...
def parse_chat_commands(user_message: str) -> Optional[dict]:
    ...
    # /video crear <tema>
    if msg.lower().startswith("/video crear ") or msg.lower().startswith(
        "/video create "
    ):
    ...
    if msg.lower().startswith("/codigo ") or msg.lower().startswith("/code "):
        ...
        if code:
            return {
                ...
                "user_feedback": "Ejecutando código Python",
            }
    ...
    if msg.lower().startswith("/fs_ver "):
        ...
        return {
            "command": "agentic_tool",
            "args": {"tool": "view_file", "filepath": path},
            "api_action": f"TOOL view_file {path}",
            "user_feedback": f"Leyendo archivo: {path}",
        }
    ...
    if msg.lower().startswith("/fs_listar "):
        ...
        return {
            "command": "agentic_tool",
            "args": {"tool": "list_dir", "directory": path},
            "api_action": f"TOOL list_dir {path}",
            "user_feedback": f"Listando directorio: {path}",
        }
    ...
    if msg.lower().startswith("/fs_buscar "):
        ...
        if len(parts) >= 3:
            return {
                ...
                "api_action": f"TOOL grep_search {parts[1]}",
                "user_feedback": f"Buscando '{parts[1]}' en {parts[2]}",
            }
    ...
    if msg.lower().startswith("/terminal "):
        ...
        return {
            "command": "agentic_tool",
            "args": {"tool": "run_command", "command": cmd, "cwd": "."},
            "api_action": f"TOOL run_command {cmd}",
            "user_feedback": f"Ejecutando terminal: {cmd}",
        }

    if msg.lower().startswith("/polish "):
        path = msg.split(" ", 1)[1].strip()
        return {
            "command": "literary_polish",
            "args": {"path": path},
            "api_action": f"TOOL polish {path}",
            "user_feedback": f"Aplicando retoque técnico (polish) a: {path}",
        }

    if msg.lower().startswith("/rewrite "):
        path = msg.split(" ", 1)[1].strip()
        return {
            "command": "literary_rewrite",
            "args": {"path": path},
            "api_action": f"TOOL rewrite {path}",
            "user_feedback": f"Iniciando reescritura profunda de: {path}",
        }

    if msg.lower().startswith("/epub "):
        path = msg.split(" ", 1)[1].strip()
        return {
            "command": "generate_epub",
            "args": {"path": path},
            "api_action": f"TOOL epub {path}",
            "user_feedback": f"Generando EPUB para: {path}",
        }

    if msg.lower().startswith("/noticia "):
        topic = msg.split(" ", 1)[1].strip()
        return {
            "command": "publish_news",
            "args": {"topic": topic},
            "api_action": f"TOOL publish_news {topic}",
            "user_feedback": f"Investigando y publicando reporte sobre: {topic}",
        }

    # ── V16.0 PRO Autonomy Commands ─────────────────────────────────────────────
    if msg.lower() in ("/autonomia", "/autonomy"):
        return {
            "command": "autonomy_status",
            "args": {},
            "api_action": "GET /v1/autonomy/status",
            "user_feedback": "Consultando estado del motor de autonomía",
        }

    if msg.lower() in ("/reflexion", "/reflection"):
        return {
            "command": "run_reflection",
            "args": {},
            "api_action": "POST /v1/reflection/trigger",
            "user_feedback": "Ejecutando ciclo de auto-introspección...",
        }

    if msg.lower() in ("/memoria", "/memory"):
        return {
            "command": "strategic_memory",
            "args": {"n": 10},
            "api_action": "GET /v1/autonomy/decisions",
            "user_feedback": "Consultando memoria estratégica",
        }
    ...
    if msg.lower() in ("/parches", "/patches"):
        return {
            ...
            "args": {},
            "api_action": "GET /v1/reflection/patches",
            "user_feedback": "Listando parches de código propuestos",
        }

    if msg.lower() in ("/reglas", "/rules"):
        return {
            "command": "invariant_rules",
            "args": {},
            "api_action": "GET /v1/autonomy/rules",
            "user_feedback": "Mostrando reglas invariantes del sistema",
        }

    if msg.lower().startswith("/decidir "):
        meta = msg.split(" ", 1)[1].strip()
        return {
            "command": "trigger_ooda",
            "args": {"meta": meta},
            "api_action": "POST /v1/autonomy/trigger",
            "user_feedback": f"Iniciando ciclo OODA para: '{meta[:60]}'",
        }
    ...
    if msg.lower().startswith("/modelo"):
        ...
        return {
            ...
            "user_feedback": (
                f"Cambiando proveedor a '{target}'"
                if target
                else "Restaurando IA a modo Automático..."
            ),
        }
    ...
    if msg.lower().startswith("/rag "):
        ...
        return {
            "command": "toggle_rag",
            "args": {"state": state},
            "api_action": "POST /v1/rag/toggle",
            "user_feedback": f"Cambiando estado RAG a: '{state}'",
        }
    ...
    if msg.lower().startswith("/fabrica "):
        ...
        return {
            "command": "run_factory",
            "args": {"prompt": prompt},
            "api_action": "POST /v1/factory/generate",
            "user_feedback": "Iniciando Fábrica de Software. Esto puede tardar varios minutos...",
        }
    ...
    if msg.lower() in ("/tareas", "/jobs"):
        return {
            ...
            "args": {},
            "api_action": "GET /v1/status",
            "user_feedback": "Consultando el Monitor de Tareas en Segundo Plano...",
        }
    ...
    if msg.lower().startswith("/investiga "):
        ...
        return {
            "command": "web_search",
            "args": {"query": query},
            "api_action": "GET /v1/tools/search",
            "user_feedback": f"Investigando en internet: '{query}'",
        }
    ...
    try:
        if cmd == "get_module_status":
                ...
                for i in range(5):
                    try:
                        ...
                        if not target or target in ("auto", "automatico", "dinamico"):
                            ...
                            return {
                                "ok": False,
                                "result_text": "✗ Comando inválido. Opciones válidas:\n- `/modelo auto` (Recomendado)\n- `/modelo local`\n- `/modelo nube`",
                            }
                        ...
                        if i == 4:
                            return {
                                "ok": False,
                                "result_text": f"✗ Error accediendo a configuración: {e}",
                            }
                        time.sleep(0.05)
            ...
            try:
                ...
                return {
                    "ok": False,
                    "result_text": f"✗ Error modificando configuración RAG: {e}",
                }
            ...
            try:
                req = urllib.request.Request(
                    f"http://127.0.0.1:{port}/v1/factory/generate",
            ...
            try:
                ...
                plan_content = provider_manager.complete(
            ...
            if result.get("success"):
                return {
                    "ok": True,
                    "result_text": f"✓ Imagen generada: /static/imagelab/{fname}\nPrompt: {args.get('prompt', '')[:80]}",
                }
            else:
                return {
                    "ok": False,
                    "result_text": f"✗ Error generando imagen: {result.get('error', 'desconocido')}",
                }
            ...
            return {
                "ok": True,
                "result_text": f"Resultados de búsqueda:\n{(r.stdout or 'Sin resultados')[:2000]}",
            }
            ...
            return {
                ...
                "result_text": output or "Ejecutado sin output.",
            }
            ...
            if breakdown:
                ...
                for prov, data in breakdown.items():
                    lines.append(
                        f"  - {prov}: ${data.get('cost', 0):.4f} ({data.get('tokens', 0):,} tokens)"
                    )
            return {"ok": True, "result_text": "\n".join(lines)}
            ...
            with _brain_lock:
                ...
                if stripped in existing:
                    return {
                        "ok": False,
                        "result_text": "Regla ya existe en el knowledge base (duplicado ignorado).",
                    }
                rules.append(entry)
            ...
            if ok:
                return {
                    "ok": True,
                    "result_text": f"✓ Regla persistida en knowledge base ({len(rules)} total).",
                }
            else:
                return {
                    "ok": False,
                    "result_text": "✗ Error escribiendo knowledge base.",
                }
            ...
            return {
                "ok": True if not res.startswith("Error") else False,
                "result_text": res,
            }
            ...
            try:
                ...
                if out_path:
                    return {
                        "ok": True,
                        "result_text": f"✓ EPUB generado con éxito:\n{out_path}",
                    }
                else:
                    return {
                        "ok": False,
                        "result_text": "✗ Fallo al generar EPUB (revisa logs).",
                    }
            except Exception as e:
                return {
                    "ok": False,
                    "result_text": f"✗ Error ejecutando epub_generator: {e}",
                }
            ...
            return {
                "ok": True,
                "result_text": f"✓ Retoque técnico completado.\nObra ensamblada y lista en:\n{result_path}",
            }
            ...
            return {
                "ok": True,
                "result_text": f"✓ Reescritura profunda completada.\nNueva obra expandida en:\n{result_path}",
            }
            ...
            try:
                ...
                return {
                    "ok": False,
                    "result_text": f"✗ Error ejecutando el reportero: {e}",
                }
        ...
        # ── V16.0 PRO Autonomy Commands ────────────────────────────────────────
            ...
            from core.self_reflection import (
                get_state as refl_state,
                _count_pending_patches,
            )

            st = ae_state()
            rst = refl_state()
            lines = [
                "╔══ GRAVITY AUTONOMY ENGINE V16.3 PRO ══╗",
            ...
            if st.get("last_decision"):
                ...
                lines.append(
                    f"  Nivel: {ld.get('level', '?')} | {ld.get('n_actions', 0)} acción(es)"
                )
            ...
            lines = [
                "Ciclo de auto-introspección completado:",
            ...
            if issues:
                ...
                for iss in issues[:5]:
                    lines.append(
                        f"  [{iss.get('severity', '?')}] {iss.get('module', '?')}: {iss.get('issue', '')[:100]}"
                    )
            ...
            lines = [
                "╔══ MEMORIA ESTRATÉGICA (últimos 30 días) ══╗",
            ...
            if not patches:
                return {
                    "ok": True,
                    "result_text": "No hay parches de código pendientes de aprobación.",
                }
            ...
            for p in patches:
                ...
                lines.append(
                    f"  → Para aprobar: POST /v1/reflection/patches/{p.get('id', '?')}/approve"
                )
            return {"ok": True, "result_text": "\n".join(lines)}
            ...
            lines.append(
                "  Estas reglas NO pueden ser modificadas por el Autonomy Engine."
            )
            lines.append("")
            ...
            if result.get("ok"):
                return {
                    "ok": True,
                    "result_text": f"✓ Ciclo OODA iniciado en background.\nMeta: '{meta}'\nConsulta el resultado con /autonomia en ~60 segundos.",
                }
            else:
                return {
                    "ok": False,
                    "result_text": f"✗ {result.get('error', 'Error desconocido')}",
                }
        ...
        return {
            "ok": False,
            "result_text": f"✗ Error ejecutando '{cmd}': {e}\n{traceback.format_exc()[:500]}",
        }
