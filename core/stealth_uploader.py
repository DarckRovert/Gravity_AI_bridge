...
class StealthUploader:
    def __init__(self):
        ...
        try:
            ...
            self._stealth = None
            log.warning("[StealthUploader] playwright-stealth no instalado. Fallback a vanilla.")
    ...
    def _launch_browser(self, p, mobile=False):
        ...
        if mobile:
            ...
            kwargs["user_agent"] = 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1'
        else:
        ...
        try:
            with sync_playwright() as p:
                ...
                if "login" in page.url or page.locator("text=Log in").count() > 0:
                    log.error("[StealthUploader] Se requiere login en TikTok. Ejecuta start_login().")
                    context.close()
                ...
                log.info("[StealthUploader] Video adjuntado. Esperando caja de texto...")
                time.sleep(5) # Dar tiempo a que cambie la interfaz
                
                # Seleccionar la caja de descripción
                caption_box = upload_frame.locator('.public-DraftEditor-content, div[contenteditable="true"]').first
                ...
                try:
                    ...
                    upload_frame.evaluate("() => { document.querySelectorAll('.react-joyride__overlay, #react-joyride-portal, [data-test-id=\"overlay\"]').forEach(e => e.remove()); }")
                except Exception as e:
                    pass
                ...
                self._human_typing(upload_frame, '.public-DraftEditor-content, div[contenteditable="true"]', caption)
                
                log.info("[StealthUploader] Descripción escrita. Buscando botón publicar...")
                time.sleep(5) # Esperar a que se habilite el botón
                
                # Click post
                post_btn = upload_frame.locator('button:has-text("Post"), button:has-text("Publicar")').last
                post_btn.click()
                
                log.info("[StealthUploader] Clic en Publicar. Esperando a que el video se suba completamente a los servidores...")
                ...
                try:
                    upload_frame.locator('text="Manage posts", text="Administrar", text="Ver perfil", text="View profile", div[class*="success"]').first.wait_for(state="visible", timeout=120000)
                    log.info("[StealthUploader] Modal de éxito detectado. El video ha sido publicado.")
                except Exception:
                    log.warning("[StealthUploader] No se detectó modal de éxito a tiempo, pero se esperó el margen de seguridad.")
    ...
    def upload_to_instagram(self, video_path: str, caption: str) -> dict:
        ...
        session_state = os.path.join(USER_DATA_DIR, "Default", "Network Persistent State")
        ...
        if not os.path.exists(USER_DATA_DIR) or not os.path.exists(cookies_file):
            msg = ("Sesión no inicializada. Ejecuta 'start_login()' primero "
        ...
        try:
            with sync_playwright() as p:
                ...
                try:
                    ...
                    log.warning("[StealthUploader] domcontentloaded timeout, reintentando con 'load'...")
                    try:
                        page.goto(
                            "https://www.instagram.com/",
                            wait_until="load",
                ...
                if page.locator('input[name="username"]').count() > 0:
                    log.error("[StealthUploader] Sesión de Instagram expirada. Requiere re-login.")
                    context.close()
                    return {
                        "ok": False,
                ...
                # Siguiente (Next)
                page.locator('button:has-text("Next"), button:has-text("Siguiente")').first.click()
                time.sleep(2)
                ...
                time.sleep(1)
                # Publicar
                share_btn = page.locator('button:has-text("Share"), button:has-text("Compartir")').first
                share_btn.click()
...
def start_login():
    ...
    with sync_playwright() as p:
        ...
        try:
            page2.goto("https://www.instagram.com/accounts/login/", wait_until="domcontentloaded", timeout=60000)
        except Exception as e:
