...
"""
    ...
    try:
        ...
        fallback = {"global_style": "cinematic, hyperrealistic, highly detailed", "characters": {}}
        with open(lore_path, "w", encoding="utf-8") as f:
...
def inject_lore_to_prompt(lore_data: dict, base_image_prompt: str) -> str:
    ...
    for char_name, char_desc in lore_data.get("characters", {}).items():
        ...
        if matched:
            ...
            face_match = re.search(r"Facial features:\s*(.*?)(?=\w+:|$)", char_desc, re.IGNORECASE)
            hair_match = re.search(r"Hair:\s*(.*?)(?=\w+:|$)", char_desc, re.IGNORECASE)
            
            extracted_traits = []
            if face_match: extracted_traits.append(face_match.group(1).strip())
            if hair_match: extracted_traits.append("Hair: " + hair_match.group(1).strip())
