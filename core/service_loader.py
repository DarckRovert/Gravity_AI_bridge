"""
╔══════════════════════════════════════════════════════════════════════════════╗
║               GRAVITY AI - CORE SERVICE LOADER V16.0 PRO                     ║
...
def load_module(module_name: str) -> Optional[Any]:
    ...
    try:
        ...
        log.error(
            f"[ServiceLoader] Failed to dynamically load module '{module_name}': {e}",
            exc_info=True,
        )
        with _status_lock:
...
def start_service(
    module_name: str, start_func_name: str = "start", *args, **kwargs
) -> bool:
    """
    ...
    with _status_lock:
        ...
        if status == "active":
            log.info(
                f"[ServiceLoader] Service '{module_name}' is already active. Skipping duplicate startup."
            )
            return True
    ...
    if not module:
        log.warning(
            f"[ServiceLoader] Skipping startup for service '{module_name}' because it could not be imported."
        )
        with _status_lock:
            _SERVICES_STATUS[module_name] = "failed"
        return False
    ...
    if not start_func:
        log.warning(
            f"[ServiceLoader] Module '{module_name}' has no '{start_func_name}' function. Marking as active (no startup needed)."
        )
        with _status_lock:
    ...
    try:
        log.info(
            f"[ServiceLoader] Starting background service: {module_name}.{start_func_name}()"
        )
        ...
        log.error(
            f"[ServiceLoader] Crash detected during startup of service '{module_name}': {e}",
            exc_info=True,
        )
        with _status_lock:
            _SERVICES_STATUS[module_name] = "failed"
