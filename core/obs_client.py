"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  GRAVITY OBS CLIENT — OBS WebSocket v5 Singleton                           ║
║  Control total de OBS Studio: escenas, fuentes, audio, stream, grabación   ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class OBSClient:
    """Singleton thread-safe para OBS WebSocket v5."""
    ...
    def _setup(self):
        ...
        t.start()
        log.info("[OBSClient] Modulo inicializado.")
    ...
    def connect(self) -> dict:
        if not OBS_AVAILABLE:
            return {"ok": False, "error": "obsws-python no instalado. pip install obsws-python"}
        
        # 1. Obtener datos de conexión de forma segura y limpiar cliente anterior
        with self._lock:
            host = self._host
            port = self._port
        ...
        # 2. Establecer la conexión de red fuera del bloqueo principal
        try:
            ...
            # 3. Guardar el cliente conectado de forma segura y refrescar estado
            with self._lock:
                self._client = client
            ...
            log.info(f"[OBSClient] Conectado a OBS {obs_version}")
            return {"ok": True, "obs_version": obs_version,
                    "ws_version": ws_version, "host": host, "port": port}
        except Exception as e:
            with self._lock:
                self._connected = False
                self._client    = None
            log.warning(f"[OBSClient] Fallo conexion: {e}")
            return {"ok": False, "error": str(e)}

    def disconnect(self):
        with self._lock:
            if self._client:
                try: self._client.base_client.ws.close()
    ...
    def _req(self, fn, *args, **kwargs):
        if not self._connected or self._client is None:
            raise ConnectionError("OBS no conectado")
        try:
            return fn(*args, **kwargs)
        except Exception as e:
            if any(k in str(e).lower() for k in ("connection", "closed", "timeout", "broken")):
                self._connected = False
                self._client    = None
            raise
    ...
    def get_status(self) -> dict:
        with self._lock:
            if not self._connected:
                return {"connected": False, "host": self._host, "port": self._port,
                        "obs_version": "", "ws_version": "", "streaming": False,
                        "recording": False, "current_scene": ""}
            try: self._refresh_locked()
            except Exception: pass
            return {"connected": True, "host": self._host, "port": self._port,
                    "obs_version": self._obs_version, "ws_version": self._ws_version,
                    "streaming": self._streaming, "recording": self._recording,
    ...
    def get_scenes(self) -> list:
        with self._lock:
            ...
            return [{"name": s["sceneName"], "uuid": s.get("sceneUuid", ""),
                     "index": s.get("sceneIndex", i)} for i, s in enumerate(resp.scenes)]

    def get_current_scene(self) -> str:
        with self._lock:
            return self._req(self._client.get_current_program_scene).current_program_scene_name
    ...
    def get_scene_items(self, scene_name: str) -> list:
        with self._lock:
            ...
            return [{"scene_item_id": it["sceneItemId"], "source_name": it["sourceName"],
                     "source_type": it.get("inputKind", it.get("sourceType", "")),
                     "enabled": it["sceneItemEnabled"], "index": it.get("sceneItemIndex", 0)}
                    for it in resp.scene_items]

    def set_item_visible(self, scene_name: str, item_id: int, visible: bool) -> dict:
        with self._lock:
            self._req(self._client.set_scene_item_enabled,
                      scene_name=scene_name, item_id=item_id, enabled=visible)
            return {"ok": True, "scene_name": scene_name, "item_id": item_id, "visible": visible}

    def toggle_item_visible(self, scene_name: str, item_id: int) -> dict:
        with self._lock:
            cur = self._req(self._client.get_scene_item_enabled, scene_name, item_id).scene_item_enabled
            self._req(self._client.set_scene_item_enabled,
                      scene_name=scene_name, item_id=item_id, enabled=not cur)
            return {"ok": True, "scene_name": scene_name, "item_id": item_id, "visible": not cur}
    ...
    def get_inputs(self) -> list:
        AUDIO_KINDS = {
            ...
            "pulse_input_capture", "pulse_output_capture", "coreaudio_input_capture",
            "coreaudio_output_capture", "ffmpeg_source", "browser_source", "vlc_source",
            "dshow_input", "ndi_source"
        }
        with self._lock:
            ...
            for inp in resp.inputs:
                ...
                entry = {"input_name": name, "kind": kind, "muted": False, "volume_db": 0.0}
                if kind in AUDIO_KINDS:
                    try:
                        entry["muted"]     = self._client.get_input_mute(name).input_muted
                        entry["volume_db"] = round(self._client.get_input_volume(name).input_volume_db, 1)
                    except Exception: pass
                inputs.append(entry)
            return inputs
    ...
    # ── Browser Sources (Gravity Spark) ────────────────────────────────────────

    def create_browser_source(self, scene_name: str, input_name: str, url: str,
            ...
            if x != 0 or y != 0:
                try:
                    self._client.set_scene_item_transform(
                        scene_name=scene_name, item_id=sid,
                        transform={"positionX": float(x), "positionY": float(y),
                                   "boundsWidth": float(width), "boundsHeight": float(height)}
                    )
                except Exception as e:
                    log.warning(f"[OBSClient] Transform error: {e}")
            return {"ok": True, "scene_name": scene_name, "input_name": input_name,
                    "scene_item_id": sid, "url": url, "width": width, "height": height}

    def update_browser_source_url(self, input_name: str, new_url: str) -> dict:
        with self._lock:
            self._req(self._client.set_input_settings,
                      name=input_name, settings={"url": new_url}, overlay=True)
            try: self._client.press_input_properties_button(input_name, "refreshnocache")
            except Exception: pass
            return {"ok": True, "input_name": input_name, "url": new_url}

    def refresh_browser_source(self, input_name: str) -> dict:
        with self._lock:
            try: self._req(self._client.press_input_properties_button, input_name, "refreshnocache")
    ...
    def get_stream_status(self) -> dict:
        with self._lock:
            ...
            return {"streaming": st.output_active, "recording": rt.output_active,
    ...
    def toggle_stream(self) -> dict:
        with self._lock:
            ...
            try:
                self._streaming = self._req(self._client.get_stream_status).output_active
            except Exception:
                self._streaming = not self._streaming  # flip optimistic
    ...
    def toggle_record(self) -> dict:
        with self._lock:
            ...
            try:
                self._recording = self._req(self._client.get_record_status).output_active
            except Exception:
                self._recording = not self._recording  # flip optimistic
...
def auto_connect_if_configured():
    """Lee config.yaml y conecta si obs_websocket.enabled=true."""
    try:
        ...
        result = _obs.connect()
        if result["ok"]:
            log.info(f"[OBSClient] Auto-conectado — OBS {result['obs_version']}")
        else:
            log.warning(f"[OBSClient] Auto-conexion fallida: {result.get('error')}")
    except Exception as e:
