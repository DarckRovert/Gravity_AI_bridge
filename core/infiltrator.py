...
class InfiltratorManager:
    def __init__(self):
        ...
        self.user_data_dir = os.path.join(BASE_DIR, "_sessions", "chrome_stealth_profile")
    ...
    def start_infiltration(self, target_url):
        ...
        _infiltrator_state["status_msg"] = f"Iniciando infiltración hacia {target_url}..."
        
        self.thread = threading.Thread(target=self._infiltration_loop, args=(target_url,), daemon=True)
        self.thread.start()
    ...
    def _infiltration_loop(self, target_url):
        ...
        try:
            ...
            while _infiltrator_state["running"]:
                ...
                _infiltrator_state["status_msg"] = f"Monitoreando orgánicamente: {self.page.url}"
                
                # Acciones aleatorias para mantener viva la sesión sin parecer bot
                sleep_time = int(random.uniform(5.0, 15.0))
                for _ in range(sleep_time):
                    if not _infiltrator_state["running"]: break
                    time.sleep(1)
                ...
                try:
                    ...
                    import base64
                    _infiltrator_state["last_screenshot"] = "data:image/jpeg;base64," + base64.b64encode(screenshot_bytes).decode('utf-8')
                except Exception:
                    pass
        ...
        try:
            if task["type"] == "freelancer_bid":
                ...
                try:
                    ...
                    _infiltrator_state["status_msg"] = "Propuesta escrita. Enviando (Full Auto-Pilot)..."
                    
                    try:
                        time.sleep(2)
                        # Buscar botón de enviar (Place Bid o Submit)
                        for btn_text in ['Place Bid', 'Submit Proposal', 'Enviar Oferta', 'Submit']:
                    ...
                    if "Timeout" in str(e):
                        log.warning(f"[Infiltrator] No se encontró la caja de texto en la oferta (Posiblemente cerró, no cumples los requisitos, o requiere relogueo). Saltando...")
                    else:
