...
class RemotionEngine:
    ...
    def render_composition(self, composition_id: str, output_name: str, props: Dict[str, Any]) -> str:
        """
        ...
        with self._lock:
            ...
            def _copy_file_with_retry(src: str, dst: Path, max_retries: int = 5, delay: float = 0.2) -> None:
                for attempt in range(max_retries):
                    try:
                        ...
                        if attempt == max_retries - 1:
                            logger.error(f"Error persistente al copiar {src} a {dst} tras {max_retries} intentos: {e}")
                            raise
            ...
            def _unlink_with_retry(path: Path, max_retries: int = 5, delay: float = 0.2) -> None:
                for attempt in range(max_retries):
                    try:
                        if path.exists():
                            path.unlink()
                        ...
                        if attempt == max_retries - 1:
                            logger.warning(f"No se pudo eliminar {path} tras {max_retries} intentos por bloqueo exclusivo.")
                        else:
                            time.sleep(delay * (2 ** attempt))
                    except Exception as ex:
                        logger.warning(f"Error inesperado al eliminar archivo {path}: {ex}")
                        return

            def _rmtree_with_retry(path: Path, max_retries: int = 5, delay: float = 0.2) -> None:
                for attempt in range(max_retries):
                    try:
                        if path.exists():
                            shutil.rmtree(path)
                        ...
                        if attempt == max_retries - 1:
                            logger.warning(f"No se pudo borrar el directorio {path} tras {max_retries} intentos por bloqueo exclusivo.")
                        else:
                            time.sleep(delay * (2 ** attempt))
                    except Exception as ex:
                        logger.warning(f"Error inesperado al borrar el directorio {path}: {ex}")
                        return
