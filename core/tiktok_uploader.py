...
def _log_attempt(
    platform: str, job_id: int, status: str, video_id: str = "", error: str = ""
) -> None:
    """
    ...
    with _social_io_lock:
        for attempt in range(5):
            try:
                ...
                if attempt == 4:
                    log.error(
                        f"[Social] Colisión persistente guardando log en disco: {e}"
                    )
                time.sleep(0.05 * (2**attempt))
            except Exception as e:
...
def upload_to_tiktok(
    job_id: int, video_path: str, title: str, tags: Optional[list[str]] = None
) -> dict:
    """
    ...
    if not tt_cfg.get("enabled", False):
        return {
            "ok": False,
            "skipped": True,
            "error": "TikTok deshabilitado (social.tiktok.enabled: false)",
        }
    ...
    if not access_token:
        log.info(
            "[TikTok] Sin credenciales API. Usando Playwright Worker (proceso aislado)..."
        )
        ...
        if res.get("ok"):
            ...
            _log_attempt(
                "tiktok", job_id, "failed", error=res.get("error", "Error stealth")
            )
        return res
        ...
        init_payload = json.dumps(
            {
                "post_info": {
                    "title": caption,
                    "privacy_level": tt_cfg.get(
                        "privacy_level", "SELF_ONLY"
                    ),  # empieza privado, luego se publica
                    "disable_duet": False,
...
def upload_to_instagram(
    ...
    if not ig_cfg.get("enabled", False):
        return {
            "ok": False,
            "skipped": True,
            "error": "Instagram deshabilitado (social.instagram.enabled: false)",
        }
    ...
    if not all([access_token, ig_user_id]):
        log.info(
            "[Instagram] Sin credenciales API. Usando Playwright Worker (proceso aislado)..."
        )
        from core._playwright_worker import run_isolated
        ...
        if res.get("ok"):
            ...
            _log_attempt(
                "instagram", job_id, "failed", error=res.get("error", "Error stealth")
            )
        return res
...
def distribute_short(
    job_id: int, shorts_path: str, title: str, tags: Optional[list[str]] = None
) -> dict:
    """
    ...
    log.info(
        f"[Social] Job #{job_id} distribuido a {ok_count}/{len(results)} plataformas."
    )
    return {"ok": ok_count > 0, "results": results, "job_id": job_id}


def distribute_short_async(
    job_id: int, shorts_path: str, title: str, tags: Optional[list[str]] = None
) -> None:
...
def get_status() -> dict:
    ...
    ig_ok = bool(
        _load_instagram_creds().get("access_token")
        and _load_instagram_creds().get("ig_user_id")
    )
    ...
    return {
        "tiktok": {
            ...
            "uploads_24h": sum(
                1
                for r in last_24h
                if r.get("platform") == "tiktok" and r.get("status") == "uploaded"
            ),
        ...
        "instagram": {
            ...
            "uploads_24h": sum(
                1
                for r in last_24h
                if r.get("platform") == "instagram" and r.get("status") == "uploaded"
            ),
