...
def _save_log(records: List[Dict[str, Any]]) -> None:
    ...
    with _revenue_io_lock:
        ...
        if last_err:
            log.error(
                f"[Revenue] Escritura atómica fallida en {REVENUE_PATH} tras 5 intentos: {last_err}"
            )
            raise last_err
...
def record_upload(
    ...
    with _revenue_io_lock:
        ...
        _save_log(records)
    log.info(
        f"[Revenue] Upload registrado: job #{job_id} | niche={niche_id} | lang={lang} | short={is_short}"
    )
        ...
        for rec in records:
            if rec.get("job_id") == job_id:
                ...
                if rec.get("is_short"):
                    ...
                    cpm = (
                        NICHE_CPM_MAP.get(niche, NICHE_CPM_MAP["_default"]) * lang_mult
                    )
            ...
            for i in range(5):
                try:
                    ...
                    if "locked" in str(e).lower():
                        ...
                        log.warning(
                            f"[Revenue] Error de base de datos SQLite en get_summary: {e}"
                        )
                        break
                except Exception as e:
                    log.warning(
                        f"[Revenue] Fallo al consultar base de datos sqlite en get_summary: {e}"
                    )
                    break
...
def get_top_jobs(limit: int = 10) -> List[Dict[str, Any]]:
    ...
    with _revenue_io_lock:
        ...
        return sorted(by_job.values(), key=lambda x: x["revenue_usd"], reverse=True)[
            :limit
        ]
