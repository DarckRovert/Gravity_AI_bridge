...
def _translate_script(scenes: List[Dict[str, Any]], target_lang: str, original_lang: str = "es") -> List[Dict[str, Any]]:
    """
...
def _execute_db_query(query: str, params: tuple = (), is_write: bool = False) -> Optional[List[Any]]:
    ...
    for attempt in range(5):
        try:
            ...
            if "locked" in str(e).lower() and attempt < 4:
                ...
                continue
            log.error(f"[LangCloner] DB locked error in query '{query}' (attempt {attempt+1}): {e}")
            raise
        except Exception as e:
...
def clone_job(source_job_id: int, target_langs: Optional[List[str]] = None) -> Dict[str, Any]:
    """
    ...
    with _cloner_lock:
        ...
        for lang in target_langs:
            ...
            try:
                ...
                probe = subprocess.run([FFMPEG_EXE, '-i', temp_short_src], capture_output=True, text=True, errors='replace', creationflags=CREATION_FLAGS)
                dur = 59
