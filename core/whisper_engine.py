...
class WhisperEngine:
    def __init__(self, model_size: str = "medium", device: str = "cpu", compute_type: str = "int8", use_npu: bool = True):
        """
        ...
        with _whisper_init_lock:
            ...
            if cache_key in _model_cache:
                logger.info(f"Reutilizando instancia de modelo cacheada (NPU={self.use_npu}).")
                self.model = _model_cache[cache_key]
                return
            
            if self.use_npu:
                try:
                    logger.info("Fase 3: Intentando activar motor ONNX Runtime para aceleración NPU/DirectML...")
                    from optimum.onnxruntime import ORTModelForSpeechSeq2Seq
                    from transformers import AutoProcessor, pipeline
                    
                    model_id = f"openai/whisper-{model_size}"
                    processor = AutoProcessor.from_pretrained(model_id)
                    model = ORTModelForSpeechSeq2Seq.from_pretrained(model_id, export=True, provider="DmlExecutionProvider")
                    self.model = pipeline("automatic-speech-recognition", model=model, tokenizer=processor.tokenizer, feature_extractor=processor.feature_extractor, return_timestamps="word")
                    _model_cache[cache_key] = self.model
                    logger.info("¡Modelo ONNX cargado exitosamente en NPU/DirectML!")
                    return
                except ImportError:
                    logger.warning("Librerías 'optimum' o 'transformers' no encontradas. Cayendo a Faster-Whisper...")
                    self.use_npu = False
                except Exception as e:
                    logger.warning(f"Error cargando NPU/DirectML: {e}. Cayendo a Faster-Whisper...")
                    self.use_npu = False
    ...
    def extract_words(self, audio_path: str, language: str = "es", initial_prompt: str = None) -> List[Dict[str, Any]]:
        """
