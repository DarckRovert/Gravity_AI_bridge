...
def extract_clips_from_video(
    ...
    try:
        probe_cmd = [
            ffmpeg_exe.replace("ffmpeg.exe", "ffprobe.exe").replace(
                "ffmpeg", "ffprobe"
            ),
            "-v",
            "error",
        ...
        log.warning(
            f"[VideoSlicer] No se pudo leer duración de {input_video_path}. Asumiendo 999s. Error: {e}"
        )
        total_duration = 999.0

    half_dur = scene_duration / 2.0
    log.info(
        f"[VideoSlicer] Procesando {input_video_path} ({total_duration}s) -> {n_scenes} clips de {half_dur}s"
    )
    ...
    for i in range(1, n_scenes + 1):
        ...
        try:
            ...
            log.info(
                f"[VideoSlicer] Clip extraído: {os.path.basename(clip_path)} (Inicio: {start_time}s)"
            )
        except subprocess.CalledProcessError as e:
            log.error(
                f"[VideoSlicer] Error extrayendo clip {i}: {e.stderr.decode('utf-8', errors='ignore')}"
            )
