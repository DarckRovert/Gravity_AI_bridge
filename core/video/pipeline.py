    ...
    with _db_lock:
        ...
        try:
            ...
            migrations = [
                ...
                ("codec", "TEXT NOT NULL DEFAULT 'h264_amf'"),
    ...
    if animation_level == 1:
        try:
            ...
            with open(
                ...
                animation_level = int(
                    _cfg_c.get("comfyui", {}).get("animation_level", 1)
                )
        except Exception:
            pass
        ...
        try:
            cur = conn.execute(
                ...
                (
                    ...
                    bgm_type,
                    quality,
                    1 if use_lore else 0,
                    fps,
                    scene_duration,
                    duration_mode,
                    float(bgm_volume),
                    codec,
                    1 if ken_burns else 0,
                    1 if intro_card else 0,
                    color_grade,
                    animation_effect,
                    int(animation_level),
                    niche_id,
            ...
            conn.close()
    log.info(
        f"[VideoStudio] Job #{job_id} encolado: {title or topic[:60]} | estilo={style} | voz='{voice_id or 'auto'}' | calidad={quality}"
    )
    return job_id
...
def get_queue_status() -> dict:
    ...
    for r in raw_history:
        job_dict = dict(r)
        if job_dict.get("status") in ("done", "completed") and job_dict.get(
            "output_path"
        ):
            ...
            with _db_lock:
                ...
                try:
                    for _pid in purge_ids:
                        _pc.execute(
                            "UPDATE video_jobs SET status='deleted', output_path=NULL WHERE id=?",
                            (_pid,),
                        )
                    _pc.commit()
                finally:
    ...
    try:
        ...
        _agg = {
            "total": _row[0] or 0,
            "completed": _row[1] or 0,
            "failed": _row[2] or 0,
            "cancelled": _row[3] or 0,
            "deleted": _row[4] or 0,
        }
...
def delete_job(job_id: int) -> dict:
    ...
    with _db_lock:
        ...
        try:
            conn.row_factory = sqlite3.Row
            row = conn.execute(
                "SELECT output_path FROM video_jobs WHERE id=?", (job_id,)
            ).fetchone()
        finally:
            conn.close()
    ...
    try:
        ...
        try:
            ...
            if not verify_token_health():
                log.warning(
                    "[VideoStudio] OAUTH_ERROR detectado. Se omitirá la subida a YouTube, pero el render local y TikTok continuarán."
                )
        ...
        if job_type == "music" and style in [
            "biomechanic_v14",
            "galactic",
            "oceanic",
            "protean",
            ...
            log.info(
                "[VideoStudio] Interceptando Pipeline: Redirigiendo a Motor GLSL V14..."
            )
            if not audio_track_path or not os.path.isfile(audio_track_path):
                raise RuntimeError(
                    "El Motor V14 requiere un archivo de audio (.mp3/.wav) válido en audio_track_path."
                )

            _update_job(
                job_id,
                progress=10,
                current_step="Analizando frecuencias de audio (Multiband)...",
            )
            ...
            _update_job(
                job_id,
                progress=30,
                current_step="Generando línea de tiempo y paletas fractales...",
            )
            ...
            if lyrics_text and len(lyrics_text.strip()) > 10:
                _update_job(
                    job_id,
                    progress=40,
                    current_step="AI Director: Analizando secciones de la letra...",
                )
                try:
                    ...
                    if ai_result and "colorsA" in ai_result:
                        log.info(
                            f"[VideoStudio] AI Director: Paletas y Timeline Narrativo dinámico aplicado ({len(ai_result['colorsA'])} frames)"
                        )
                        ...
                        if "timeline" in ai_result and len(ai_result["timeline"]) > 0:
                            ...
                            log.info(
                                f"[VideoStudio] AI Director: {len(timeline)} escenas narrativas mapeadas con éxito."
                            )
                    else:
                        log.warning(
                            "[VideoStudio] AI Director no devolvió secciones válidas, usando análisis acústico global de fallback."
                        )
            ...
            final_path = os.path.join(
                OUTPUT_DIR, f"video_{job_id}_{safe_topic}_V14_{ts}.mp4"
            )
            ...
            _tgt_w = (
                int(_res_parts[0])
                if len(_res_parts) == 2 and _res_parts[0].isdigit()
                else DEFAULT_IMG_W
            )
            _tgt_h = (
                int(_res_parts[1])
                if len(_res_parts) == 2 and _res_parts[1].isdigit()
                else DEFAULT_IMG_H
            )
            ...
            if style in [
                ...
                log.info(
                    f"[VideoStudio] Estilo '{style}': Omitiendo generación de imágenes AI 2D (Render 100% Shader)."
                )
                ...
                _update_job(
                    job_id,
                    progress=45,
                    current_step="Generando fondos cinematográficos AI-First...",
                )
                background_images = None
                try:
                    from core.video.ai_scene_generator import generate_scene_images

                    background_images = generate_scene_images(
                        timeline, w=_tgt_w, h=_tgt_h, colorsA=colorsA
                    )
                    if not background_images:
                        background_images = [None] * len(timeline)
                except Exception as e_bg:
                    log.warning(
                        f"[VideoStudio] Fallo al generar imágenes AI-First: {e_bg}"
                    )
                    background_images = [None] * len(timeline)
            ...
            _update_job(
                job_id,
                progress=48,
                current_step="Extrayendo letras y generando subtítulos ASS...",
            )
            ass_path = None
            try:
                ...
                ass_path, real_lyrics = generate_ass_subtitles(
                    audio_track_path, tmp_ass, lyrics_text=lyrics_text
                )
            ...
            _update_job(
                job_id,
                progress=50,
                current_step=f"Renderizando {style.upper()} V14 (Compute Shaders)...",
            )
            render_v14_compute_video(
                ...
                speed_multiplier=(
                    speed_mult_arr if speed_mult_arr is not None else speed_mult
                ),
            ...
            if not os.path.isfile(final_path):
                raise RuntimeError(
                    "El renderizador V14 falló y no devolvió ningún MP4."
                )
            ...
            if style in [
                ...
                _update_job(
                    job_id,
                    progress=75,
                    current_step=f"Renderizando NATIVO VERTICAL {style.upper()} V17 (GPU)...",
                )
                ...
                if ass_path:
                    try:
                        ...
                        tmp_ass_vert = os.path.join(
                            OUTPUT_DIR, f"temp_{job_id}_lyrics_vertical.ass"
                        )
                        ass_path_vertical, _ = generate_ass_subtitles(
                            audio_track_path,
                            tmp_ass_vert,
                            tgt_w=_tgt_h,
                            tgt_h=_tgt_w,
                        ...
                        log.warning(
                            f"[VideoStudio] Fallo al generar subtítulos ASS Verticales: {e_sub_vert}"
                        )
                ...
                render_v14_compute_video(
                    ...
                    turbulence=(
                        turb_mult_arr if turb_mult_arr is not None else turb_mult
                    ),
                    background_images=background_images,
            ...
            scenes = [
                {
                    "title": "V14 Music Video",
                    "image_prompt": "Audio Reactive V14",
                    "narration": "",
                }
            ]
            thumb_path = ""
            ...
            scenes, visual_anchor, generated_title = _generate_script(
                topic, n_scenes, style, narration_lang, use_lore
            )
            if not scenes:
        ...
        if v14_bypass:
            ...
            log.info(
                f"[VideoStudio] Visual Anchor del job #{job_id}: '{visual_anchor[:80]}'"
            )
        ...
        if intro_card and not v14_bypass:
            ...
            if _create_title_card(
                title or "Video Promocional",
                style_info.get("label", "Cinema Studio"),
                intro_path,
                w_ic,
                h_ic,
                fps,
        ...
        for i, scene in enumerate(scenes):
            ...
            with _lock:
                if _current_job:
                    ...
                    _current_job["current_step"] = (
                        f"[{scene_num}/{n_scenes}] Generando imagen..."
                    )
                    _current_job["current_scene"] = scene_num

            img_path = _generate_scene_image(
                anchored_prompt, scene_num, job_id, job_seed, style, resolution
            )

            if not img_path:
                placeholder = os.path.join(
                    job_dir, f"scene_{scene_num:02d}_placeholder.png"
                )
                w_ph, h_ph = 1216, 832
                if "x" in resolution:
                    try:
            ...
            with _lock:
                if _current_job:
                    _current_job["progress"] = pct
                    _current_job["current_step"] = (
                        f"[{scene_num}/{n_scenes}] Generando audio..."
                    )
                    _current_job["current_scene"] = scene_num
            ...
            if job_type == "music":
                ...
                audio_ok = (
                    _generate_audio(narration, audio_path, voice_speed, voice_id)
                    if narration
                    else False
                )
            ...
            with _lock:
                if _current_job:
                    _current_job["progress"] = pct
                    _current_job["current_step"] = (
                        f"[{scene_num}/{n_scenes}] Ensamblando clip..."
                    )
                    _current_job["current_scene"] = scene_num
            ...
            if animation_level >= 1:
                ...
                try:
                    ...
                    if (
                        len(user_video_clips) >= scene_num
                        and user_video_clips[scene_num - 1]
                    ):
                        _ai_dur = _ai_dur / 2.0
                    ...
                    if _anim_l15:
                        ...
                        log.info(
                            f"[VideoStudio] [MAI-L1.5] Escena {scene_num}: variaciones Pollinations → {os.path.basename(_anim_l15)}"
                        )
            ...
            if (
                animation_level >= 2
                and _animated_src == img_path
                and not img_path.endswith(".mp4")
            ):
                ...
                try:
                    ...
                    if _anim_mp4:
                        ...
                        log.info(
                            f"[VideoStudio] [MAI-L2] Escena {scene_num}: animación ComfyUI → {os.path.basename(_anim_mp4)}"
                        )
                    else:
                        log.info(
                            f"[VideoStudio] [MAI-L2] ComfyUI no disponible. Fallback a L1 ({effective_animation})."
                        )
            ...
            if len(user_video_clips) >= scene_num and user_video_clips[scene_num - 1]:
                ...
                w = (
                    int(_res_wh[0])
                    if len(_res_wh) == 2 and _res_wh[0].isdigit()
                    else DEFAULT_IMG_W
                )
                h = (
                    int(_res_wh[1])
                    if len(_res_wh) == 2 and _res_wh[1].isdigit()
                    else DEFAULT_IMG_H
                )
                try:
                    if not _animated_src.endswith(".mp4"):
                        ...
                        img_vid = os.path.join(
                            job_dir, f"scene_{scene_num:02d}_img.mp4"
                        )
                        subprocess.run(
                            [
                                FFMPEG_EXE,
                    ...
                    subprocess.run(
                        [
                            ...
                            _animated_src,
                            "-filter_complex",
                            f"[0:v]scale={w}:{h}:force_original_aspect_ratio=increase,crop={w}:{h},setsar=1/1,fps=24[v0];[1:v]scale={w}:{h}:force_original_aspect_ratio=increase,crop={w}:{h},setsar=1/1,fps=24[v1];[v0][v1]concat=n=2:v=1:a=0[v]",
                            "-map",
                            "[v]",
                            "-c:v",
                            codec,
                    ...
                    log.warning(
                        f"[VideoStudio] Fallo al crear híbrido para escena {scene_num}: {e}"
                    )
                ...
                try:
                    ...
                    if subtitles:
                        try:
                            ...
                            _words = _we.extract_words(
                                audio_path, language=narration_lang[:2]
                            )
                        except Exception as wh_err:
                            log.warning(f"[VideoStudio] Whisper Engine fail: {wh_err}")
                except Exception as e:
                    log.warning(
                        f"[VideoStudio] Error calculando tiempos para {audio_path}: {e}"
                    )
            ...
            with _lock:
                if _current_job and scene_num not in _current_job.get(
                    "scenes_done", []
                ):
        ...
        _update_job(
            job_id,
            progress=95,
            current_step="Renderizando video principal con Remotion...",
        )
        with _lock:
            if _current_job:
        ...
        if not v14_bypass:
            ...
            final_path = os.path.join(
                OUTPUT_DIR, f"video_{job_id}_{safe_topic}_{ts}.mp4"
            )
        ...
        if v14_bypass:
            ...
            total_duration_frames = sum(
                s.get("durationInFrames", 150) for s in scenes_payload
            )
            ...
            try:
                ...
                main_rendered_path = r_engine.render_composition(
                    "LongTemplate", output_name, long_props
                )
        ...
        if main_video_rendered:
            if v14_bypass:
                ...
                if needs_concat:
                    if _concatenate_clips(
                        clip_paths, final_path, bgm_type, bgm_volume, codec, resolution
                    ):
                        pass
                    else:
                        import shutil
                        ...
                        log.warning(
                            "[VideoStudio] Fallback: video sin BGM/intro (concat falló)."
                        )
                else:
                    import shutil
            ...
            if v14_bypass:
                ...
                job_type == "music"
                and audio_track_path
                and os.path.isfile(audio_track_path)
            ):
                import subprocess
                ...
                try:
                    ...
                    if apply_beat_synced_fx(
                        final_path, audio_track_path, beat_fx_path, fps
                    ):
                        import shutil

                        shutil.move(beat_fx_path, final_path)
                        log.info(
                            "[VideoStudio] Beat-Synced FX (Glitch rítmico) aplicados con éxito al video."
                        )
        ...
        if render_ok:
            ...
            if job_type == "music":
                ...
                try:
                    ...
                    shutil.copy2(
                        final_path,
                        os.path.join(music_out_dir, os.path.basename(final_path)),
                    )
                except Exception as c_e:
                    log.warning(
                        f"[VideoStudio] No se pudo copiar a carpeta musical: {c_e}"
                    )
            ...
            try:
                if style in [
                    ...
                    log.info(
                        "[VideoStudio] Shorts nativos verticales 9:16 ya fueron generados por la GPU en el paso anterior."
                    )
                    _shorts_path = final_path.replace(".mp4", "_short.mp4")
                    try:
                        if job_type == "music":
                            ...
                            try:
                                ...
                                shutil.copy2(
                                    _shorts_path,
                                    os.path.join(
                                        r"F:\PROYECTO VIDEOCLIP MUSICAL\output",
                                        os.path.basename(_shorts_path),
                                    ),
                                )
                                ...
                                log.info(
                                    "[VideoStudio] Copia Dual (Horizontal + Vertical) enviada al directorio de música."
                                )
                            except Exception:
                                pass
                    except Exception as e_crop:
                        log.warning(
                            f"[VideoStudio] Error generando GLSL Short con FFMPEG: {e_crop}"
                        )
                else:
                    log.info(
                        "[VideoStudio] Iniciando generación de Shorts interactivos Multi-Parte con Remotion y Whisper..."
                    )
                    from core.whisper_engine import WhisperEngine
                    from core.remotion_engine import RemotionEngine
                    import subprocess
                    ...
                    num_parts = max(
                        1,
                        int(total_dur // chunk_length)
                        + (1 if total_dur % chunk_length > 5 else 0),
                    )
                    ...
                    for part_idx in range(num_parts):
                        ...
                        part_suffix = (
                            f"_short_part{part_idx + 1}.mp4"
                            if num_parts > 1
                            else "_short.mp4"
                        )
                        _shorts_path = final_path.replace(".mp4", part_suffix)
                        temp_short_src = final_path.replace(
                            ".mp4", f"_temp_short_{part_idx}.mp4"
                        )
                        ...
                        if not os.path.isfile(temp_short_src):
                            log.warning(
                                f"[VideoStudio] No se pudo cortar fragmento {part_idx+1}: {temp_short_src}"
                            )
                            continue
                        ...
                        try:
                            words_data = w_engine.extract_words(
                                temp_short_src, language=narration_lang[:2]
                            )
                        except Exception as w_e:
                            log.warning(
                                f"[VideoStudio] Whisper falló en parte {part_idx+1}: {w_e}. Usando palabras vacías."
                            )
                            words_data = []
                        ...
                        try:
                            ...
                            output_name = os.path.basename(_shorts_path).replace(
                                ".mp4", ""
                            )
                            ...
                            if os.path.isfile(rendered_mp4):
                                ...
                                log.info(
                                    f"[VideoStudio] Short (Parte {part_idx + 1}/{num_parts}) generado: {_shorts_path}"
                                )
                                if job_type == "music":
                                    try:
                                        shutil.copy2(
                                        ...
                                        log.warning(
                                            f"[VideoStudio] No se pudo copiar short a carpeta musical: {c_e}"
                                        )
                        except Exception as rem_e:
                            log.error(
                                f"[VideoStudio] Error Remotion en parte {part_idx+1}: {rem_e}"
                            )
                        finally:
                            try:
                ...
                try:
                    ...
                    if (
                        "_shorts_path" in locals()
                        and _shorts_path
                        and os.path.isfile(_shorts_path)
                    ):
            ...
            if use_lore:
                try:
                    ...
                    with open(lore_path, "a", encoding="utf-8") as f:
                        safe_title = (
                            topic
                            if len(topic) < 100 and not topic.startswith("http")
                            else (title or "Historia_Generada")
                        )
                        f.write(f"\n\n=== HISTORIA: {safe_title} ===\n")
                        for s in scenes:
            ...
            raise RuntimeError(
                "Remotion render falló. Verifica los logs de RemotionEngine para detalles."
            )

    except Exception as e:
        if str(e) == "Proceso cancelado por el usuario.":
            log.info(
                f"[VideoStudio] Job #{job_id} detenido exitosamente por cancelación."
            )
        else:
            now = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
            _update_job(
        ...
        try:
            ...
            if def_prov in cfg.get("ai_engines", {}):
                log.info(
                    f"[VideoStudio] [Auto-Wake] Renderizado completado. Restaurando motor de IA local: {def_prov}"
                )
                import threading

                threading.Thread(
                    target=start_engine, args=(def_prov,), daemon=True
                ).start()
        except Exception as e:
            log.warning(
                f"[VideoStudio] Fallo al restaurar motor de IA tras render: {e}"
            )
...
def start() -> None:
    ...
    with _lock:
        ...
        _init_db()
        t = threading.Thread(
            target=_worker_loop, name="GravityVideoWorker", daemon=True
        )
        t.start()
        log.info(
            "[VideoStudio] Worker daemon iniciado (Gravity Studio ULTRA V16.0 PRO - Modular Pipeline)."
        )
