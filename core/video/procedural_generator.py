...
def _render_julia_fractal_3d(
    w: int, h: int, seed: int, palette: tuple, t: float = 0.0, energy: float = 0.0
) -> Image.Image:
...
def _render_synthwave_3d(
    ...
    sky_bot = np.clip(
        (pa + pb * np.cos(6.28318 * (pc * 0.5 + pd))) * 255, 0, 255
    ).astype(int)
    neon_col = np.clip(
        (pa + pb * np.cos(6.28318 * (pc * 0.8 + pd))) * 255, 0, 255
    ).astype(int)
    ...
    sun_r = int(
        sun_r_base
        + np.sin(t * np.pi * 4) * (sun_r_base * 0.02)
        + (energy * sun_r_base * 0.2)
    )
    ...
    draw.ellipse(
        [sun_x - sun_r, sun_y - sun_r, sun_x + sun_r, sun_y + sun_r], fill=sun_glow
    )
    ...
    for y in range(sun_y - sun_r + cut_offset, sun_y + sun_r, 12):
        ...
        if y > horizon_y - 20:
            thickness *= 2
        draw.line(
            [(sun_x - sun_r - 50, y), (sun_x + sun_r + 50, y)],
            fill=(5, 5, 10),
            width=thickness,
        )
    ...
    for gz in range(grid_size_z - 1):
        ...
        for gx in range(-grid_size_x // 2, grid_size_x // 2):
            if (gx, gz) not in vertices or (gx, gz + 1) not in vertices:
                continue
            ...
            if v1[1] > h and v2[1] > h and v3[1] > h and v4[1] > h:
                continue
    ...
    glow = img.filter(ImageFilter.GaussianBlur(radius=glow_radius)).point(
        lambda p: p * (0.6 + energy * 0.3)
    )
    img = ImageChops.add(img, glow)
    return img


def _render_space_odyssey(
    w: int, h: int, seed: int, palette: tuple, t: float = 0.0, energy: float = 0.0
) -> Image.Image:
    ...
    def generate_noise(shape, scale, rng_obj):
        ...
        x_t = x_t * x_t * (3 - 2 * x_t)
        y_t = y_t * y_t * (3 - 2 * y_t)
        x_idx0, x_idx1 = np.clip(x_idx, 0, noise.shape[1] - 1), np.clip(
            x_idx + 1, 0, noise.shape[1] - 1
        )
        y_idx0, y_idx1 = np.clip(y_idx, 0, noise.shape[0] - 1), np.clip(
            y_idx + 1, 0, noise.shape[0] - 1
        )
    ...
    img = Image.fromarray(img_arr)
    glow = img.filter(ImageFilter.GaussianBlur(radius=15)).point(
        lambda p: p * (0.4 + energy * 0.2)
    )
...
def _render_frame_worker(args):
    ...
    if any(kw in prompt_lower for kw in ["cyber", "neon", "city", "grid", "synthwave"]):
        ...
        kw in prompt_lower
        for kw in ["fractal", "horror", "dark", "complex", "magic", "gold"]
    ):
...
def generate_procedural_video(
    ...
    print(
        f"\n[Motor Matemático V5] PARALELO: Usando {n_cores} núcleos de CPU (Límite RAM).",
        file=sys.stderr,
    )
    print(
        f"[Motor Matemático V5] Renderizando {total_frames} frames ({w}x{h}) para {os.path.basename(out_mp4)}...",
        file=sys.stderr,
    )
    ...
    try:
        ...
        with multiprocessing.Pool(processes=n_cores) as pool:
            for frame_idx, img_bytes in pool.imap(
                _render_frame_worker, args_list, chunksize=1
            ):
                proc.stdin.write(img_bytes)
                if frame_idx % 24 == 0:
                    e = audio_energy[frame_idx]
                    print(
                        f"  Frame {frame_idx}/{total_frames} ({(frame_idx/total_frames)*100:.1f}%) [Energy: {e:.3f}]",
                        file=sys.stderr,
                    )
...
def generate_art(prompt: str, seed: int, w: int, h: int, out_path: str) -> str:
    ...
    try:
        if any(
            kw in prompt_lower for kw in ["cyber", "neon", "city", "grid", "synthwave"]
        ):
