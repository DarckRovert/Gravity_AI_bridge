...
# --- SHADERS GLSL V10 (LÓGICA ESPACIAL Y PBR EN PERSONAJE) ---
...
# LIBRERÍA COMÚN SDF PERSONAJE (AHORA CON CÁLCULO DE NORMALES)
CHARACTER_LIB = '''
...
float sdCharacter(vec3 p, float t, int pose) {
    ...
    if(pose == 2) headPos = vec3(0.0, 0.2, 1.2); // Volando aerodinámico
    ...
    float torso = sdCapsule(p, torsoTop, torsoBot, 0.28);
    ...
    float legL1 = sdCapsule(p, leftHip, leftKnee, 0.13);
    float legL2 = sdCapsule(p, leftKnee, leftFoot, 0.1);
    float legR1 = sdCapsule(p, rightHip, rightKnee, 0.13);
    ...
    float armL1 = sdCapsule(p, leftShoulder, leftElbow, 0.09);
    float armL2 = sdCapsule(p, leftElbow, leftHand, 0.07);
    float armR1 = sdCapsule(p, rightShoulder, rightElbow, 0.09);
    float armR2 = sdCapsule(p, rightElbow, rightHand, 0.07);
...
vec3 calcCharNormal(vec3 p, float t, int pose) {
    ...
    return normalize( e.xyy*sdCharacter( p + e.xyy, t, pose ) + 
					  e.yyx*sdCharacter( p + e.yyx, t, pose ) + 
					  e.yxy*sdCharacter( p + e.yxy, t, pose ) + 
					  e.xxx*sdCharacter( p + e.xxx, t, pose ) );
}
'''

# 1. SPACE ODYSSEY V10 (Framing Corregido + Volumen 3D)
SPACE_ODYSSEY_FS = '''
#version 330
out vec4 fragColor;
in vec2 uv;
...
vec3 charOffset = vec3(0.0, 0.0, -3.0); // Personaje a 3 uds del agujero

vec2 map(vec3 p) {
    // Escenario (Agujero Negro en el Origen 0,0,0)
    ...
    // Suelo infinito debajo del personaje para que no flote
    float floorDist = p.y + 1.2; 
    world = min(world, floorDist);
    
    // Personaje Parado mirando el agujero
    vec3 cp = p - charOffset; 
    cp.y += 0.2; // Ajuste para que pise el suelo
...
void main() {
    ...
    // CÁMARA (LÓGICA V10): Atrás y ligeramente arriba, mirando al origen
    vec3 ro = vec3(0.0, 1.0, -6.5); 
    pR(ro.xz, pan * 0.5); 
    
    vec3 target = vec3(0.0, 0.0, 0.0); // Mirar al agujero negro
    ...
    if(t<max_d) {
        vec3 pos = ro + rd*t;
        if(mat_id == 1.0) {
            // LÓGICA V10: Cálculo de Normales (Volumen) para el Personaje
            vec3 nor = calcCharNormal(pos - charOffset + vec3(0.0, 0.2, 0.0), time, 0);
            vec3 lig = normalize(vec3(0.0, 1.0, 2.0)); // Luz viniendo desde el agujero negro
            float dif = clamp(dot(nor, lig), 0.0, 1.0);
            float spe = pow(clamp(dot(reflect(rd, nor), lig), 0.0, 1.0), 16.0);
            
            vec3 baseCol = colorB * 0.5;
            col = baseCol * dif + spe * colorB * high;
            // Backlight emissive
            col += colorB * clamp(1.0 - dot(nor, -rd), 0.0, 1.0) * 0.5;
        } else {
            // Agujero negro
            if(length(pos)<1.01 && pos.y > -1.0) col = vec3(0.0);
                ...
                col = colorA * 0.05 * smoothstep(0.0, 0.1, grid);
            }
        }
    }
...
# 2. JULIA FRACTAL V10 (Adiós Moonwalking, Avance Físico Constante)
JULIA_FS = '''
#version 330
out vec4 fragColor;
in vec2 uv;
...
// Variable global de avance
float forwardTravel() { return time * 3.0; }

vec2 map(vec3 p) {
    // El mundo es repetitivo o avanza
    vec3 jp = p;
    jp.y -= 1.0; // Fractal como "suelo" y "techo"
    jp.z = mod(jp.z, 8.0) - 4.0; // Repetir el fractal en Z infinitamente
    
    float world = juliaSDF(jp, vec4(sin(time*0.5)*0.5, cos(time*0.3)*0.5, mid*0.5, -0.2));
    
    // Personaje caminando (Pose 1). Avanza por el eje Z global.
    vec3 charPos = vec3(0.0, 0.0, forwardTravel());
    vec3 cp = p - charPos;
    cp.y += 0.5; // Tocar el suelo fractal
...
void main() {
    vec2 p = (gl_FragCoord.xy * 2.0 - resolution.xy) / resolution.y;
    // CÁMARA (LÓGICA V10): Cámara avanza físicamente siguiendo al personaje
    float camZ = forwardTravel() - 3.5;
    vec3 ro = vec3(0.0, 0.5, camZ); 
    pR(ro.xz, pan * 0.5); pR(ro.yz, sin(time*0.5)*0.05); // Menos vibración de cámara
    
    vec3 ww = normalize(vec3(0.0, 0.0, 1.0)); // Mirar directamente adelante
    vec3 uu = normalize(cross(ww, vec3(0.0, 1.0, 0.0)));
    vec3 vv = normalize(cross(uu, ww));
    ...
    if(t<max_d) {
        vec3 pos = ro + rd*t;
        if(mat_id == 1.0) {
            // LÓGICA V10: PBR en el caminante
            vec3 nor = calcCharNormal(pos - vec3(0.0, -0.5, forwardTravel()), time*(1.0 + bass*2.0), 1);
            vec3 lig = normalize(vec3(1.0, 1.0, -1.0));
            float dif = clamp(dot(nor, lig), 0.0, 1.0);
            float spe = pow(clamp(dot(reflect(rd, nor), lig), 0.0, 1.0), 16.0);
            
            col = colorB * (0.2 + dif*0.8) + spe * high;
            col += mix(colorA, colorB, 0.5) * clamp(1.0 - dot(nor, -rd), 0.0, 1.0); // Fresnel
        } else {
...
# 3. QUANTUM TUNNEL V10 (Volumen 3D y Reflejos de Neón)
QUANTUM_TUNNEL_FS = '''
#version 330
out vec4 fragColor;
in vec2 uv;
...
vec2 map(vec3 p) {
    ...
    // Personaje cayendo adelante de la cámara
    vec3 charPos = vec3(0.0, 0.0, forwardTravel() + 2.5);
    // Suave vibración natural, no caótica
    charPos.x += sin(time*4.0)*0.2;
    charPos.y += cos(time*3.0)*0.2;
...
void main() {
    vec2 p = (gl_FragCoord.xy * 2.0 - resolution.xy) / resolution.y;
    // Cámara avanzando
    vec3 ro = vec3(0.0, 0.0, forwardTravel());
    ro.x += sin(time)*0.3 + pan; // Oscilación suave de la cámara
    
    vec3 rd = normalize(vec3(p.x, p.y, 1.0));
    pR(rd.xy, sin(time)*0.1); // Rollo de cámara suave
    
    float t = 0.0, max_d = 30.0, glow = 0.0;
    float mat_id = 0.0;
    ...
    if(t<max_d) {
        vec3 pos = ro + rd*t;
        if(mat_id == 1.0) {
            // LÓGICA V10: El personaje refleja los anillos de neón del túnel
            vec3 charPos = vec3(0.0, 0.0, forwardTravel() + 2.5);
            ...
            // Fresnel super brillante emulando traje metálico reflectante
...
# (Nebula y Mandelbulb quedan como planos B intactos para intercalar)
# Mantenemos las definiciones simples para evitar romper V8 compatibilidad
...
def render_v10_video(timeline: list, multiband: dict, colorsA: np.ndarray, colorsB: np.ndarray, w: int, h: int, fps: int, out_mp4: str, audio_path: str):
    ...
    print(f"\n[🚀 Motor V10] INICIANDO RENDER CON LÓGICA Y PBR AVANZADO...")
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        print(f"Error renderizando video V10 GPU: {e}", file=sys.stderr)
    finally:
    ...
    print(f"[✅ Motor V10] Lógica Reparada y PBR Renderizado en: {out_mp4}", file=sys.stderr)
    return out_mp4
