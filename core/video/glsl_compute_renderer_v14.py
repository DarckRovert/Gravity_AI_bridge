...
COMPUTE_PARTICLES = open(
    os.path.join(os.path.dirname(__file__), "shaders", "compute_particles.glsl"),
    "r",
    encoding="utf-8",
).read()
...
PARTICLES_VS = open(
    os.path.join(os.path.dirname(__file__), "shaders", "particles_vs.glsl"),
    "r",
    encoding="utf-8",
).read()

PARTICLES_FS = open(
    os.path.join(os.path.dirname(__file__), "shaders", "particles_fs.glsl"),
    "r",
    encoding="utf-8",
).read()
...
VERTEX_SHADER = open(
    os.path.join(os.path.dirname(__file__), "shaders", "vertex_shader.glsl"),
    "r",
    encoding="utf-8",
).read()

COSMOS_LIB = open(
    os.path.join(os.path.dirname(__file__), "shaders", "cosmos_lib.glsl"),
    "r",
    encoding="utf-8",
).read()
...
# 1. SPACE ODYSSEY V13
SPACE_ODYSSEY_FS = open(
    os.path.join(os.path.dirname(__file__), "shaders", "space_odyssey_fs.glsl"),
    "r",
    encoding="utf-8",
).read()
...
# 2. JULIA FRACTAL V13
JULIA_FS = open(
    os.path.join(os.path.dirname(__file__), "shaders", "julia_fs.glsl"),
    "r",
    encoding="utf-8",
).read()

# 3. QUANTUM TUNNEL V13
QUANTUM_TUNNEL_FS = open(
    os.path.join(os.path.dirname(__file__), "shaders", "quantum_tunnel_fs.glsl"),
    "r",
    encoding="utf-8",
).read()

# POST PROCESS FS (Cinematic Overhaul)
POST_PROCESS_FS = open(
    os.path.join(os.path.dirname(__file__), "shaders", "post_process_fs.glsl"),
    "r",
    encoding="utf-8",
).read()

# --- GARGANTUA (RELATIVIDAD GENERAL E INTERESTELAR) ---
INTERSTELLAR_FS = open(
    os.path.join(os.path.dirname(__file__), "shaders", "interstellar_fs.glsl"),
    "r",
    encoding="utf-8",
).read()
...
KIFS_FS = open(
    os.path.join(os.path.dirname(__file__), "shaders", "kifs_fs.glsl"),
    "r",
    encoding="utf-8",
).read()
...
NEON_FLUID_FS = open(
    os.path.join(os.path.dirname(__file__), "shaders", "neon_fluid_fs.glsl"),
    "r",
    encoding="utf-8",
).read()

# --- JOYA 3: NUCLEO ORGANICO (RAYTRACED SUBSURFACE SCATTERING) ---
ORGANIC_CORE_FS = open(
    os.path.join(os.path.dirname(__file__), "shaders", "organic_core_fs.glsl"),
    "r",
    encoding="utf-8",
).read()

# --- JOYA 4: TURING PATTERNS (REACTION-DIFFUSION BIOLUMINISCENTE) ---
TURING_PATTERNS_FS = open(
    os.path.join(os.path.dirname(__file__), "shaders", "turing_patterns_fs.glsl"),
    "r",
    encoding="utf-8",
).read()
...
COMPOSITE_FS = open(
    os.path.join(os.path.dirname(__file__), "shaders", "composite_fs.glsl"),
    "r",
    encoding="utf-8",
).read()

MANDELBULB_FS = open(
    os.path.join(os.path.dirname(__file__), "shaders", "mandelbulb_fs.glsl"),
    "r",
    encoding="utf-8",
).read()

OCEANIC_FS = open(
    os.path.join(os.path.dirname(__file__), "shaders", "oceanic_fs.glsl"),
    "r",
    encoding="utf-8",
).read()
...
PROTEAN_FS = open(
    os.path.join(os.path.dirname(__file__), "shaders", "protean_fs.glsl"),
    "r",
    encoding="utf-8",
).read()
...
INCA_MATH_FS = open(
    os.path.join(os.path.dirname(__file__), "shaders", "inca_math_fs.glsl"),
    "r",
    encoding="utf-8",
).read()


def render_v14_compute_video(
    timeline: list,
    multiband: dict,
    colorsA: np.ndarray,
    colorsB: np.ndarray,
    ...
    print(
        "\n[🧠 Motor V14] Compilando Compute Shader (XDNA Simulation)...",
        file=sys.stderr,
    )
    try:
        ...
        prog_particles = ctx.program(
            vertex_shader=PARTICLES_VS, fragment_shader=PARTICLES_FS
        )
        vao_particles = ctx.vertex_array(prog_particles, [])

        print(
            f"[🧠 Motor V14] SSBO Creado y Render de Partículas ({NUM_PARTICLES}) listo.",
            file=sys.stderr,
        )
    except Exception as e:
    ...
    try:
        for frame_idx in range(total_frames):
            ...
            def render_pass(engine_name, fbo, pose_val, bg_tex, t_val):
                ...
                if compute_active:
                    if "time" in compute_prog:
                        compute_prog["time"].value = t_val
                    if "bass" in compute_prog:
                        compute_prog["bass"].value = b
                    if "mid" in compute_prog:
                        compute_prog["mid"].value = m
                ...
                if compute_active:
                    ...
                    if "time" in prog_particles:
                        prog_particles["time"].value = t_val
                    if "bass" in prog_particles:
                        prog_particles["bass"].value = b
                    if "pan" in prog_particles:
                        prog_particles["pan"].value = pan
                    if "high" in prog_particles:
                        prog_particles["high"].value = hg
                    if "aspect" in prog_particles:
                        prog_particles["aspect"].value = w / float(h)

                    engine_val = 0
                    if engine_name == "space_odyssey":
                        engine_val = 1
                    elif engine_name == "mandelbulb":
                        engine_val = 2
                    if "engine_id" in prog_particles:
                        prog_particles["engine_id"].value = engine_val
