...
def apply_beat_synced_fx(video_path: str, audio_path: str, output_path: str, fps: int = 24) -> bool:
    ...
    if not os.path.isfile(video_path) or not os.path.isfile(audio_path):
        log.error("[BeatSyncer] Faltan archivos de video o audio para la sincronización.")
        return False

    log.info("[BeatSyncer] Analizando frecuencias de audio para inyectar transiciones rítmicas...")
    multiband = extract_multiband_energy(audio_path, fps)
    
    bass_energy = multiband.get("bass", [])
    if not len(bass_energy):
        log.warning("[BeatSyncer] No se detectó energía de graves. Saltando FX rítmico.")
        return False
