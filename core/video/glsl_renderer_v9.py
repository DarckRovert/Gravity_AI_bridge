...
# --- SHADERS GLSL V9 (EL CAMINANTE) ---
# Hemos inyectado matemáticas orgánicas para crear un personaje SDF que responde a la canción.
...
float sdCharacter(vec3 p, float t, int pose) {
    p *= 1.5; // Escala
    ...
    if(pose == 2) headPos = vec3(0.0, 0.3, 1.2); // Volando
    float head = length(p - headPos) - 0.25;
    
    // Torso
    vec3 torsoTop = vec3(0.0, 1.3, 0.0);
    vec3 torsoBot = vec3(0.0, 0.5, 0.0);
    if(pose == 2) { torsoTop = vec3(0.0, 0.0, 0.8); torsoBot = vec3(0.0, 0.0, -0.2); }
    float torso = sdCapsule(p, torsoTop, torsoBot, 0.25);
    
    // Piernas
    vec3 leftHip = torsoBot + vec3(-0.15, 0.0, 0.0);
    vec3 rightHip = torsoBot + vec3(0.15, 0.0, 0.0);
    
    vec3 leftKnee = leftHip + vec3(0.0, -0.4, sin(walk_cycle)*0.3);
    vec3 rightKnee = rightHip + vec3(0.0, -0.4, -sin(walk_cycle)*0.3);
    vec3 leftFoot = leftKnee + vec3(0.0, -0.5, sin(walk_cycle+0.5)*0.2);
    vec3 rightFoot = rightKnee + vec3(0.0, -0.5, -sin(walk_cycle+0.5)*0.2);
    
    if(pose == 2) { // Volando
        leftKnee = leftHip + vec3(-0.1, -0.1, -0.5);
        leftFoot = leftKnee + vec3(-0.1, 0.1, -0.4);
        rightKnee = rightHip + vec3(0.1, -0.1, -0.5);
        rightFoot = rightKnee + vec3(0.1, 0.1, -0.4);
    }
    
    float legL1 = sdCapsule(p, leftHip, leftKnee, 0.12);
    float legL2 = sdCapsule(p, leftKnee, leftFoot, 0.1);
    float legR1 = sdCapsule(p, rightHip, rightKnee, 0.12);
    float legR2 = sdCapsule(p, rightKnee, rightFoot, 0.1);
    
    // Brazos
    vec3 leftShoulder = torsoTop + vec3(-0.35, -0.1, 0.0);
    vec3 rightShoulder = torsoTop + vec3(0.35, -0.1, 0.0);
    
    vec3 leftElbow = leftShoulder + vec3(-0.1, -0.4, -sin(walk_cycle)*0.3);
    vec3 rightElbow = rightShoulder + vec3(0.1, -0.4, sin(walk_cycle)*0.3);
    ...
    if(pose == 2) { // Pose Superman
        leftElbow = leftShoulder + vec3(-0.2, 0.0, 0.5);
        leftHand = leftElbow + vec3(-0.1, 0.0, 0.5);
        rightElbow = rightShoulder + vec3(0.2, 0.0, 0.5);
        rightHand = rightElbow + vec3(0.1, 0.0, 0.5);
    }
    
    float armL1 = sdCapsule(p, leftShoulder, leftElbow, 0.08);
    float armL2 = sdCapsule(p, leftElbow, leftHand, 0.06);
    float armR1 = sdCapsule(p, rightShoulder, rightElbow, 0.08);
    float armR2 = sdCapsule(p, rightElbow, rightHand, 0.06);
    
    float d = smin(head, torso, 0.1);
    d = smin(d, smin(legL1, legL2, 0.1), 0.1);
    d = smin(d, smin(legR1, legR2, 0.1), 0.1);
    d = smin(d, smin(armL1, armL2, 0.1), 0.1);
    d = smin(d, smin(armR1, armR2, 0.1), 0.1);
    
    return d / 1.5;
}
'''

# 1. SPACE ODYSSEY V9 (Agujero Negro + Personaje Parado)
SPACE_ODYSSEY_FS = '''
#version 330
out vec4 fragColor;
in vec2 uv;
...
vec2 map(vec3 p) {
    // Escenario
    vec3 sp = p; sp.y -= 1.0;
    float sphere = length(sp) - 1.0;
    vec3 dp = sp;
    pR(dp.xz, time * 0.5); pR(dp.xy, mid * 0.5);
    float disk = sdTorus(dp, vec2(1.8 + bass*0.5, 0.05 + mid*0.1));
    float world = min(sphere, disk);
    
    // Personaje (Pose 0 = Parado frente al agujero)
    vec3 cp = p - vec3(0.0, -1.0, -3.0); // Posición del personaje
    float charD = sdCharacter(cp, time, 0);
    
    // return vec2(distancia, id_material: 0=mundo, 1=personaje)
    if(charD < world) return vec2(charD, 1.0);
...
void main() {
    ...
    pR(ro.xz, pan * 0.5); // Leve rotación
    vec3 ww = normalize(vec3(0.0, 1.0, 0.0) - ro);
    vec3 uu = normalize(cross(ww, vec3(0.0, 1.0, 0.0)));
    vec3 vv = normalize(cross(uu, ww));
    vec3 rd = normalize(p.x * uu + p.y * vv + 1.2 * ww);
    
    float t = 0.0; float max_d = 20.0; vec3 col = vec3(0.0); float glow = 0.0; float char_glow = 0.0;
    float mat_id = 0.0;
    ...
    for(int i=0; i<64; i++) {
        ...
        t += res.x;
        // Brillo del disco
        if(res.y == 0.0 && length(pos - vec3(0.0,1.0,0.0))>1.2) glow += 0.02 / (0.05 + res.x*res.x) * (1.0 + bass*2.0);
        // Brillo del personaje (Ente de luz)
        if(res.y == 1.0) char_glow += 0.03 / (0.01 + res.x*res.x);
    }
    
    if(t<max_d) {
        vec3 pos = ro + rd*t;
        if(mat_id == 1.0) {
            // El personaje brilla intensamente
            col = mix(colorB, vec3(1.0), 0.8);
        } else {
            // Agujero negro
            if(length(pos - vec3(0.0,1.0,0.0))<1.01) col = vec3(0.0);
    ...
    col += colorB * char_glow * 0.1; // Halo del personaje
    fragColor = vec4(col, 1.0);
}
'''

# 2. JULIA FRACTAL V9 (Personaje Caminando)
JULIA_FS = '''
#version 330
out vec4 fragColor;
in vec2 uv;
...
vec2 map(vec3 p) {
    float world = juliaSDF(p, vec4(sin(time*0.5)*0.5, cos(time*0.3)*0.5, mid*0.5, -0.2));
    
    // Personaje caminando (Pose 1). Lo situamos un poco más cerca de la cámara
    vec3 cp = p - vec3(0.0, 0.0, -1.5);
    float charD = sdCharacter(cp, time*(1.0 + bass*2.0), 1);
...
void main() {
    vec2 p = (gl_FragCoord.xy * 2.0 - resolution.xy) / resolution.y;
    // Cámara en 3ra persona siguiendo al personaje
    vec3 ro = vec3(0.0, 0.5, -3.0); 
    pR(ro.xz, pan * 0.5); pR(ro.yz, sin(time*0.1)*0.2);
    
    vec3 ww = normalize(vec3(0.0, 0.0, -1.0) - ro);
    vec3 uu = normalize(cross(ww, vec3(0.0, 1.0, 0.0)));
    vec3 vv = normalize(cross(uu, ww));
    vec3 rd = normalize(p.x * uu + p.y * vv + 1.2 * ww);
    ...
    for(int i=0; i<64; i++) {
        ...
        t += res.x; iter++;
        if(res.y == 1.0) char_glow += 0.02 / (0.01 + res.x*res.x);
    }
    
    vec3 col = colorA * 0.1;
    if(t<max_d) {
        if(mat_id == 1.0) {
            col = mix(colorB, vec3(1.0), 0.9); // Personaje súper brillante
        } else {
    ...
    col += colorB * char_glow * 0.15; // Reflejo en el ambiente
    fragColor = vec4(col, 1.0);
}
'''

# 3. QUANTUM TUNNEL V9 (Personaje Volando/Caída Libre)
QUANTUM_TUNNEL_FS = '''
#version 330
out vec4 fragColor;
in vec2 uv;
...
vec2 map(vec3 p) {
    ...
    // Personaje en el centro, volando (Pose 2)
    // El túnel se mueve rápido, el personaje "físicamente" en GLSL está estático en Z, 
    // dando la ilusión de volar hacia adelante.
    vec3 cp = p - vec3(sin(time*2.0)*0.5, 0.0, time*(5.0 + mid*5.0) + 1.5);
    // Añadimos vibración al personaje en drops
    cp.x += sin(time*50.0)*bass*0.1;
    cp.y += cos(time*50.0)*bass*0.1;
    
    float charD = sdCharacter(cp, time, 2);
...
void main() {
    vec2 p = (gl_FragCoord.xy * 2.0 - resolution.xy) / resolution.y;
    vec3 ro = vec3(0.0, 0.0, time*(5.0 + mid*5.0));
    ro.x += sin(time*2.0)*0.5 + pan;
    
    vec3 rd = normalize(vec3(p.x, p.y, 1.0));
    pR(rd.xy, sin(time)*0.2);
    
    float t = 0.0, max_d = 30.0, glow = 0.0, char_glow = 0.0;
    float mat_id = 0.0;
    ...
    if(t<max_d) {
        if(mat_id == 1.0) {
            col = vec3(1.0); // Personaje modo luz pura por la hipervelocidad
        } else {
            col = mix(colorA, colorB, fract(t*0.1));
        }
    }
    col += mix(colorA, colorB, 0.5) * glow * (0.5 + high);
    col += mix(vec3(1.0), colorB, 0.5) * char_glow * 0.2;
    
    fragColor = vec4(col, 1.0);
}
'''

# Mantendremos Mandelbulb y Nebula como planos de detalle (B-Roll) sin personaje
MANDELBULB_FS = '''
#version 330
out vec4 fragColor;
in vec2 uv;
...
float mandelbulbSDF(vec3 pos) {
    ...
    float Power = 8.0 + sin(time*0.2)*2.0 + bass*4.0; 
    for (int i=0; i<8; i++) {
...
vec3 calcNormal(vec3 pos) {
    ...
    return normalize( e.xyy*mandelbulbSDF( pos + e.xyy ) + e.yyx*mandelbulbSDF( pos + e.yyx ) + 
					  e.yxy*mandelbulbSDF( pos + e.yxy ) + e.xxx*mandelbulbSDF( pos + e.xxx ) );
}

void main() {
    vec2 p = (gl_FragCoord.xy * 2.0 - resolution.xy) / resolution.y;
    vec3 ro = vec3(0.0, 0.0, -2.5); pR(ro.xz, time * 0.1 + pan); pR(ro.xy, time * 0.05);
    vec3 ww = normalize(-ro); vec3 uu = normalize(cross(ww, vec3(0.0, 1.0, 0.0)));
    vec3 vv = normalize(cross(uu, ww)); vec3 rd = normalize(p.x * uu + p.y * vv + 1.0 * ww);
    ...
    if(t<max_d) {
        vec3 pos = ro + rd*t; vec3 nor = calcNormal(pos);
...
float noise(vec3 p) {
    ...
    vec2 rg = fract(sin((uv+0.5)*0.014)*292.0); 
...
void main() {
    ...
    for(int i=0; i<50; i++) {
        ...
        if(den > 0.01) {
            ...
            col *= mix(1.0, 3.0, bass); col += high * colorB * 0.5; 
            vec4 src = vec4(col * den, den); src.rgb *= src.a;
            sum = sum + src*(1.0 - sum.a); 
        }
        if(sum.a > 0.99) break;
        t += 0.1;
    }
...
def render_v9_video(timeline: list, multiband: dict, colorsA: np.ndarray, colorsB: np.ndarray, w: int, h: int, fps: int, out_mp4: str, audio_path: str):
    ...
    print(f"\n[🚀 Motor V9] INICIANDO NARRATIVA CINEMÁTICA CON PERSONAJE SDF...")
    ...
    try:
        for frame_idx in range(total_frames):
            ...
            if frame_idx % (fps*2) == 0:
                print(f"  Frame {frame_idx}/{total_frames} ({(frame_idx/total_frames)*100:.1f}%) [Actor in: {engine_1} | Trans: {transition_t:.2f}]", file=sys.stderr)
        ...
        import traceback
        traceback.print_exc()
        print(f"Error renderizando video V9 GPU: {e}", file=sys.stderr)
    finally:
    ...
    print(f"[✅ Motor V9] Película con Personaje Matemático renderizada en: {out_mp4}", file=sys.stderr)
    return out_mp4
