...
# --- SHADERS GLSL V12 (KINEMÁTICA PERFECTA Y RIGGING FK) ---
...
vec2 sdCharacter(vec3 p, float t, int pose) {
    ...
    float wC = (pose == 1) ? t * 5.0 : 0.0;
    
    // Cabeza (Cráneo Elíptico)
    vec3 headPos = vec3(0.0, 1.65, 0.0);
    vec3 neckBot = vec3(0.0, 1.35, 0.0);
    if(pose == 2) { headPos = vec3(0.0, 0.2, 1.3); neckBot = vec3(0.0, 0.1, 0.9); }
    
    vec3 hp = p - headPos;
    float head = (length(hp * vec3(1.2, 0.9, 1.0)) - 0.22) * 0.8;
    float neck = sdCapsule(p, headPos, neckBot, 0.08);
    ...
    float abdomen = sdCapsule(p, chestMid, torsoBot, 0.22); 
    
    // V12: Core Espinal (Reactor en la espalda para cámara en 3ra Persona)
    vec3 corePos = mix(torsoTop, chestMid, 0.7);
    corePos.z += (pose == 2) ? 0.0 : -0.25; // Z Negativo = Espalda visible hacia cámara
    if (pose == 2) corePos.y += 0.35; 
    
    float coreDist = sdCapsule(p, corePos + vec3(0.0, 0.2, 0.0), corePos - vec3(0.0, 0.2, 0.0), 0.08);
    if (pose == 2) coreDist = sdCapsule(p, corePos + vec3(0.0, 0.0, 0.2), corePos - vec3(0.0, 0.0, 0.2), 0.08);

    // KINEMÁTICA V12: Rotaciones Rígidas (Longitud Preservada)
    vec3 leftHip = torsoBot + vec3(-0.16, 0.0, 0.0);
    vec3 rightHip = torsoBot + vec3(0.16, 0.0, 0.0);
    
    float angleL = sin(wC) * 0.6; 
    float angleR = sin(wC + 3.1415) * 0.6;
    
    // Pierna Izquierda
    vec3 lKneePos = vec3(0.0, -0.45, 0.0);
    if(pose == 1) lKneePos.yz *= rot(angleL);
    vec3 leftKnee = leftHip + lKneePos;
    
    vec3 lFootPos = vec3(0.0, -0.45, 0.0);
    if(pose == 1) {
        float kneeBendL = max(0.0, sin(wC - 1.5) * 0.8);
        lFootPos.yz *= rot(angleL + kneeBendL); // Rodilla se dobla
    }
    vec3 leftFoot = leftKnee + lFootPos;
    
    // Pierna Derecha
    vec3 rKneePos = vec3(0.0, -0.45, 0.0);
    if(pose == 1) rKneePos.yz *= rot(angleR);
    vec3 rightKnee = rightHip + rKneePos;
    
    vec3 rFootPos = vec3(0.0, -0.45, 0.0);
    if(pose == 1) {
        float kneeBendR = max(0.0, sin(wC + 3.1415 - 1.5) * 0.8);
        rFootPos.yz *= rot(angleR + kneeBendR);
    }
    vec3 rightFoot = rightKnee + rFootPos;
    
    if(pose == 2) { 
        leftKnee = leftHip + vec3(-0.1, -0.1, -0.5); leftFoot = leftKnee + vec3(0.0, 0.0, -0.45);
        rightKnee = rightHip + vec3(0.1, -0.1, -0.5); rightFoot = rightKnee + vec3(0.0, 0.0, -0.45);
    }
    ...
    // Brazos 
    vec3 leftShoulder = torsoTop + vec3(-0.4, -0.1, 0.0);
    vec3 rightShoulder = torsoTop + vec3(0.4, -0.1, 0.0);
    
    float armAngleL = -angleL * 0.8;
    float armAngleR = -angleR * 0.8;
    
    vec3 lElbowPos = vec3(-0.1, -0.35, 0.0);
    if (pose == 1) lElbowPos.yz *= rot(armAngleL);
    vec3 leftElbow = leftShoulder + lElbowPos;
    
    vec3 lHandPos = vec3(-0.05, -0.35, 0.0);
    if (pose == 1) {
        float elbowBendL = max(0.0, -sin(wC)*0.5 + 0.2);
        lHandPos.yz *= rot(armAngleL - elbowBendL);
    }
    vec3 leftHand = leftElbow + lHandPos;
    
    vec3 rElbowPos = vec3(0.1, -0.35, 0.0);
    if (pose == 1) rElbowPos.yz *= rot(armAngleR);
    vec3 rightElbow = rightShoulder + rElbowPos;
    
    vec3 rHandPos = vec3(0.05, -0.35, 0.0);
    if (pose == 1) {
        float elbowBendR = max(0.0, -sin(wC + 3.1415)*0.5 + 0.2);
        rHandPos.yz *= rot(armAngleR - elbowBendR);
    }
    vec3 rightHand = rightElbow + rHandPos;
    
    if(pose == 2) { 
        leftElbow = leftShoulder + vec3(-0.1, 0.0, 0.5); leftHand = leftElbow + vec3(0.0, 0.0, 0.4);
        rightElbow = rightShoulder + vec3(0.1, 0.0, 0.5); rightHand = rightElbow + vec3(0.0, 0.0, 0.4);
    }
    
    float bicepL = sdCapsule(p, leftShoulder, leftElbow, 0.10);
    float forearmL = sdCapsule(p, leftElbow, leftHand, 0.07);
    float bicepR = sdCapsule(p, rightShoulder, rightElbow, 0.10);
    float forearmR = sdCapsule(p, rightElbow, rightHand, 0.07);
    ...
    if (coreDist < body) return vec2(coreDist, 2.0); 
    return vec2(body, 1.0); 
}
...
void main() {
    ...
    if(t<max_d) {
        ...
        } else if(mat_id == 2.0) { // Reactor Espinal Brillante
            col = vec3(1.0) + colorA * (1.0 + bass*3.0); 
        } else {
...
def render_v12_video(timeline: list, multiband: dict, colorsA: np.ndarray, colorsB: np.ndarray, w: int, h: int, fps: int, out_mp4: str, audio_path: str):
    ...
    print(f"\n[🚀 Motor V12] INICIANDO RENDER KINEMÁTICO (RIGGING MATRICIAL)...")
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        print(f"Error renderizando video V12 GPU: {e}", file=sys.stderr)
    finally:
    ...
    print(f"[✅ Motor V12] KINEMÁTICA PERFECTA RENDERIZADA EN: {out_mp4}", file=sys.stderr)
    return out_mp4
