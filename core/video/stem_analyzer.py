...
def analyze_stems(
    audio_path: str, fps: int = 24, target_duration_sec: Optional[float] = None
) -> dict:
    """
    ...
    """
    log.info(
        f"[StemAnalyzer] Extrayendo Stems Matemáticos vía HPSS para {os.path.basename(audio_path)}..."
    )
    ...
    tempo, beat_frames = librosa.beat.beat_track(
        y=y_mono, sr=sr, hop_length=int(sr / fps)
    )
    ...
    valid_beats = np.clip(
        np.round(beat_frames * (target_frames / len(beats))), 0, target_frames - 1
    ).astype(int)
