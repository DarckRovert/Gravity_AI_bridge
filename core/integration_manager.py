"""
╔══════════════════════════════════════════════════════════════════════════════╗
║               GRAVITY AI - CORE INTEGRATION MANAGER V16.0 PRO                ║
    ...
    with _manager_lock:
        if not os.path.exists(INTEGRATIONS_DIR):
            try:
                ...
                log.info(
                    f"[IntegrationManager] Created empty integrations directory at: {INTEGRATIONS_DIR}"
                )
            except Exception as e:
                log.error(
                    f"[IntegrationManager] Failed to create integrations directory: {e}"
                )
                return {}
        ...
        log.info(
            "[IntegrationManager] Scanning '/integrations' directory for plugins..."
        )
        ...
        for item in os.listdir(INTEGRATIONS_DIR):
            ...
            try:
                ...
                log.debug(
                    f"[IntegrationManager] Loading plugin package: {plugin_module_name}"
                )
                ...
                for name, obj in inspect.getmembers(module):
                    if (
                        inspect.isclass(obj)
                        and issubclass(obj, GravityIntegration)
                        and obj is not GravityIntegration
                    ):
                        try:
                            instance = obj()
                            ...
                            log.info(
                                f"[IntegrationManager] Successfully registered plugin: '{instance.name}' ({instance.description})"
                            )
                            found_class = True
                        except Exception as inst_err:
                            log.error(
                                f"[IntegrationManager] Failed to instantiate plugin class {name} from {plugin_module_name}: {inst_err}"
                            )

                if not found_class:
                    log.debug(
                        f"[IntegrationManager] Loaded module '{plugin_module_name}' but found no valid GravityIntegration subclass."
                    )

            except Exception as mod_err:
                log.error(
                    f"[IntegrationManager] Error loading plugin folder '{item}': {mod_err}",
                    exc_info=True,
                )
...
def initialize_all() -> bool:
    ...
    with _manager_lock:
        ...
        for name, data in _LOADED_INTEGRATIONS.items():
            ...
            try:
                log.info(
                    f"[IntegrationManager] Initializing integration plugin: '{name}'..."
                )
                success = data["instance"].initialize()
                if success:
                    data["status"] = "active"
                    log.info(
                        f"[IntegrationManager] Plugin '{name}' initialized successfully."
                    )
                else:
                    data["status"] = "failed"
                    log.error(
                        f"[IntegrationManager] Plugin '{name}' reported initialization failure."
                    )
                    all_ok = False
            except Exception as e:
                data["status"] = "failed"
                log.error(
                    f"[IntegrationManager] Crash occurred during initialization of plugin '{name}': {e}",
                    exc_info=True,
                )
                all_ok = False
        ...
        for name, data in _LOADED_INTEGRATIONS.items():
            ...
            try:
                ...
                if success:
                    ...
                    data["status"] = "failed"
                    log.error(
                        f"[IntegrationManager] Plugin '{name}' reported shutdown errors."
                    )
                    all_ok = False
            except Exception as e:
                data["status"] = "failed"
                log.error(
                    f"[IntegrationManager] Crash occurred during shutdown of plugin '{name}': {e}",
                    exc_info=True,
                )
                all_ok = False
...
def get_plugin(name: str) -> Optional[GravityIntegration]:
    ...
    with _manager_lock:
        ...
        return (
            ...
            if plugin_data and plugin_data["status"] == "active"
            else None
        )
