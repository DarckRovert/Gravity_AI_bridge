...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] [SwarmNode] %(message)s")
...
class SwarmNodeHandler(BaseHTTPRequestHandler):
        ...
        if self.path == "/v1/swarm/engine":
            if action == "start":
                ...
                if not path or not os.path.exists(path):
                    self._send_response(400, {"success": False, "error": f"Invalid path on node: {path}"})
                    return
    ...
    def start_engine(self, provider_name: str, path: str) -> dict:
        try:
            ...
            if path.endswith(".bat"):
                subprocess.Popen(["cmd.exe", "/c", path], creationflags=proc_flags, cwd=os.path.dirname(path), env=env, close_fds=True)
            else:
                subprocess.Popen([path], creationflags=proc_flags, cwd=os.path.dirname(path), env=env, close_fds=True)
                
            log.info(f"Started {provider_name} via {path}")
            return {"success": True, "message": f"{provider_name} iniciado en Swarm Node."}
    ...
    def stop_engine(self, provider_name: str) -> dict:
        ...
        if killed > 0:
            ...
            return {"success": True, "message": f"{provider_name} detenido ({killed} procesos cerrados en Nodo)."}
        else:
