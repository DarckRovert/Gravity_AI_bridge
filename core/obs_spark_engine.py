...
def _call_local_llm(prompt: str, temperature: float = 0.6) -> str:
    """Llama al provider_manager de Gravity para generar el HTML del overlay."""
    ...
    if not provider:
        try:
            ...
            if nvidia:
                provider = "Nvidia NIM"
                # Usar Llama 3.3 70B o Nemotron 70B de Nvidia
...
def generate_overlay(
    ...
    """
    Genera un overlay HTML con IA local y lo inyecta en OBS como Browser Source.

    Args:
        prompt:      Descripción en lenguaje natural del overlay deseado.
        scene_name:  Nombre de la escena OBS. Si vacío, usa la escena activa.
        width/height: Dimensiones del Browser Source en píxeles.
        x/y:         Posición en la escena.
        bridge_port: Puerto de Gravity Bridge para construir la URL del overlay.

    Returns:
        dict con ok, overlay_id, input_name, scene_item_id, preview_url, html_path
    """
    ...
    TEMPLATE_PROMPTS = {
        ...
        "meta_subs": [
            "meta subs", "meta_subs",
            "barra de meta de subs con hitos (25%, 50%, 100%). script js incrementa el progreso constantemente. al alcanzar un hito, la barra cambia de color vibrante y emite confeti css/canvas localmente. el texto muestra \"meta: x/100\" actualizándose fluidamente. estilo moderno, bordes redondeados (pill-shape)."
        ],
        "reloj_scifi": [
    ...
    if not html_content:
        try:
            # Usar temperatura más alta (0.8) si el usuario explícitamente desactivó la caché para fomentar creatividad
    ...
    try:
        ...
        # Si OBS falla, limpiar el archivo
...
def edit_overlay(overlay_id: str, new_prompt: str, bridge_port: int = 7860) -> dict:
    """Regenera un overlay existente con un nuevo prompt, manteniendo el mismo Browser Source."""
    from core.obs_client import get_client
    obs = get_client()
...
def remove_overlay(overlay_id: str) -> dict:
    """Elimina un overlay de OBS y del disco."""
    from core.obs_client import get_client
    obs = get_client()
...
def get_overlays() -> list:
    """Retorna la lista de overlays activos."""
    with _overlays_lock:
        return [
            {
...
def get_overlay_html(overlay_id: str) -> str | None:
    """Retorna el contenido HTML de un overlay guardado en disco."""
