...
def fetch_reddit_json():
    ...
    try:
        ...
        if e.code == 403:
            log.warning(
                "[BountyHunter] Reddit bloqueó la conexión (403). Ignorando por ahora."
            )
        else:
...
def _init_bounties_file():
    ...
    if not os.path.exists(BOUNTIES_FILE):
        with open(BOUNTIES_FILE, "w", encoding="utf-8") as f:
            ...
            f.write(
                "Aquí irán apareciendo los trabajos técnicos extraídos de Freelancer.com con su propuesta de venta lista para que copies, pegues y cobres.\n\n"
            )
            f.write("---\n\n")
        ...
        try:
            ...
            for post in posts:
                ...
                if is_bounty_relevant(title, text):
                    log.info(
                        f"[BountyHunter] 🎯 Nuevo micro-trabajo detectado: {title}"
                    )
                    ...
                    proposal = re.sub(
                        r"<think>.*?</think>", "", proposal, flags=re.DOTALL
                    ).strip()

                    if proposal.startswith("Error"):
                        log.warning(
                            f"[BountyHunter] Ignorando trabajo debido a fallo del LLM: {proposal}"
                        )
                    ...
                    with open(BOUNTIES_FILE, "a", encoding="utf-8") as f:
                        ...
                        f.write(
                            f"**Plataforma:** {platform} | **Detectado:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
                        )
                        f.write(f"**Enlace original:** {url}\n\n")
                        f.write("### Descripción Original del Cliente\n")
                    ...
                    if platform == "Freelancer.com":
                        try:
                            ...
                            infiltrator.manager.queue_task(
                                {
                                    "type": "freelancer_bid",
                                    "url": url,
                                    "proposal": proposal,
                            ...
                            log.info(
                                "[BountyHunter] 🚀 Oferta inyectada al Infiltrador en Full Auto-Pilot."
                            )
                        except Exception as e:
            ...
            if new_bounties > 0:
                log.info(
                    f"[BountyHunter] Se han añadido {new_bounties} nuevas propuestas listas a {BOUNTIES_FILE}."
                )
