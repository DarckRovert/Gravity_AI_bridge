    ...
    with _yt_io_lock:
        for attempt in range(5):
            try:
                ...
                if attempt == 4:
                    log.error(
                        f"[YouTube] Colisión persistente en lectura de quota: {e}"
                    )
    ...
    with _yt_io_lock:
        for attempt in range(5):
            try:
                ...
                if attempt == 4:
                    log.error(
                        f"[YouTube] No se pudo incrementar la quota por bloqueo de archivo persistente: {e}"
                    )
                time.sleep(0.05 * (2**attempt))
            except Exception as e:
    ...
    with _yt_io_lock:
        for attempt in range(5):
            try:
                ...
                if attempt == 4:
                    log.error(
                        f"[YouTube] Error crítico guardando OAuth tras múltiples reintentos: {e}"
                    )
                time.sleep(0.05 * (2**attempt))
            except Exception as e:
...
def _refresh_access_token(oauth: Dict[str, Any]) -> Optional[str]:
    ...
    with _token_lock:
        ...
        if not all([refresh_token, client_id, client_secret]):
            log.error(
                "[YouTube] Faltan credenciales OAuth (client_id / client_secret / refresh_token)."
            )
            return None
        ...
        for attempt in range(4):
            try:
                ...
                log.warning(
                    f"[YouTube-Shield] Error {e.code} refrescando token. Reintentando ({attempt+1}/4)..."
                )
                time.sleep(2**attempt)  # Backoff: 1s, 2s, 4s, 8s

            except Exception as e:
                log.warning(
                    f"[YouTube-Shield] Error inesperado de red: {e}. Reintentando ({attempt+1}/4)..."
                )
...
def verify_token_health() -> bool:
    ...
    if not oauth or not oauth.get("refresh_token"):
        log.info(
            "[YouTube-Shield] No hay cuenta de YouTube vinculada. Render local permitido."
        )
    ...
    if token:
        log.info(
            "[YouTube-Shield] ✅ Salud del token perfecta. Vía libre para renderizar."
        )
        return True
    else:
        log.error(
...
def get_access_token() -> Optional[str]:
    ...
    if not oauth:
        log.warning(
            "[YouTube] Sin credenciales OAuth. Completa el flujo /v1/youtube/auth/url primero."
        )
        return None
...
def _build_seo_description(
    ...
    hashtags = (
        ...
        if tags
        else "#IA #Tecnologia #DarckRovert"
    )

    _i18n = {
        "es": {
            "intro1": "En este video exploramos uno de los temas más fascinantes del momento.",
            "intro2": "Contenido generado con Gravity AI Bridge — el pipeline de producción autónoma de DarckRovert.",
            "content": "📌 CONTENIDO DEL VIDEO",
            "follow": "🔔 SÍGUENOS",
            "warn1": "⚠️ Este canal sube contenido nuevo cada día.",
            "warn2": "Suscríbete y activa la campana para no perderte nada.",
            "chap_intro": "Introducción",
            "chap_dev": "Desarrollo del tema",
            "chap_key": "Puntos clave",
            "chap_concl": "Conclusión",
        },
        "en": {
            "intro1": "In this video we explore one of the most fascinating topics of the moment.",
            "intro2": "Content generated with Gravity AI Bridge — DarckRovert's autonomous production pipeline.",
            "content": "📌 VIDEO CONTENT",
            "follow": "🔔 FOLLOW US",
            "warn1": "⚠️ This channel uploads new content every day.",
            "warn2": "Subscribe and hit the bell to not miss anything.",
            "chap_intro": "Introduction",
            "chap_dev": "Topic development",
            "chap_key": "Key points",
            "chap_concl": "Conclusion",
        },
        "pt": {
            "intro1": "Neste vídeo exploramos um dos temas mais fascinantes do momento.",
            "intro2": "Conteúdo gerado com Gravity AI Bridge — o pipeline de produção autônoma de DarckRovert.",
            "content": "📌 CONTEÚDO DO VÍDEO",
            "follow": "🔔 SIGA-NOS",
            "warn1": "⚠️ Este canal envia conteúdo novo todos os dias.",
            "warn2": "Inscreva-se e ative o sininho para não perder nada.",
            "chap_intro": "Introdução",
            "chap_dev": "Desenvolvimento do tema",
            "chap_key": "Pontos chave",
            "chap_concl": "Conclusão",
        },
        "fr": {
            "intro1": "Dans cette vidéo, nous explorons l'un des sujets les plus fascinants du moment.",
            "intro2": "Contenu généré avec Gravity AI Bridge — le pipeline de production autonome de DarckRovert.",
            "content": "📌 CONTENU DE LA VIDÉO",
            "follow": "🔔 SUIVEZ-NOUS",
            "warn1": "⚠️ Cette chaîne met en ligne de nouveaux contenus chaque jour.",
            "warn2": "Abonnez-vous et activez la cloche pour ne rien rater.",
            "chap_intro": "Introduction",
            "chap_dev": "Développement du sujet",
            "chap_key": "Points clés",
            "chap_concl": "Conclusion",
        },
        "de": {
            "intro1": "In diesem Video erkunden wir eines der faszinierendsten Themen des Moments.",
            "intro2": "Inhalte generiert mit Gravity AI Bridge — DarckRoverts autonome Produktionspipeline.",
            "content": "📌 VIDEOINHALT",
            "follow": "🔔 FOLGE UNS",
            "warn1": "⚠️ Dieser Kanal lädt jeden Tag neue Inhalte hoch.",
            "warn2": "Abonnieren Sie und aktivieren Sie die Glocke, um nichts zu verpassen.",
            "chap_intro": "Einführung",
            "chap_dev": "Themenentwicklung",
            "chap_key": "Wichtige Punkte",
            "chap_concl": "Fazit",
        },
    }
    ...
    if scene_count > 0:
        ...
        if scene_count >= 4:
            chapters.append(
                f"{_fmt_ts(secs_per_scene * (scene_count // 2))} {t['chap_key']}"
            )
        if scene_count >= 6:
            chapters.append(
                f"{_fmt_ts(secs_per_scene * (scene_count - 1))} {t['chap_concl']}"
            )
...
def _update_job_youtube(
    job_id: int, video_id: str, url: str, status: str, seo_description: str = ""
) -> None:
    try:
...
def _generate_ctr_thumbnail(video_path: str, title: str, output_jpg: str) -> bool:
    ...
    if not os.path.isfile(FFMPEG_EXE) or not os.path.isfile(video_path):
        return False
    safe_title: str = (
        title.replace("'", "").replace(":", "").replace("%", "")[:45].upper()
    )
    vf: str = (
...
def _build_metadata_bytes(
    title: str, description: str, tags: list, category_id: str, privacy: str
) -> bytes:
    meta = {
        "snippet": {
            "title": title[:100],
...
def _resumable_upload(
    access_token: str, video_path: str, metadata_bytes: bytes
) -> Optional[str]:
        ...
        with open(video_path, "rb") as vf:
            while True:
                ...
                if resp_chunk.status == 200 or resp_chunk.status == 201:
                    ...
                    if pct % 20 == 0:
                        log.debug(
                            f"[YouTube] Progreso: {pct}% ({uploaded/(1024*1024):.1f} MB)"
                        )
                    continue
                else:
...
def upload_video(
    ...
    if not cfg.get("enabled", False):
        return {
            "ok": False,
            "error": "YouTube deshabilitado en config.yaml",
            "skipped": True,
        }
    ...
    affiliate_block = ""
    try:
        from core.affiliate_manager import (
            build_affiliate_block,
            log_affiliate_injection,
            get_affiliate_links,
        )
    ...
    if upload_short and cfg.get("upload_shorts", True):
        ...
        if os.path.isfile(shorts_path):
            log.info(
                f"[YouTube] Usando Short pre-generado por Remotion: {os.path.basename(shorts_path)}"
            )
...
def upload_job_async(
    ...
    def _run():
        log.info(
            f"[YouTube] Upload async job #{job_id}: {os.path.basename(video_path)}"
        )
        result = upload_video(
            job_id=job_id,
...
def get_upload_status(job_id: int) -> dict:
    try:
        ...
        return {
            ...
            "shorts_url": (
                f"https://www.youtube.com/shorts/{row['shorts_video_id']}"
                if row["shorts_video_id"]
                else ""
            ),
        }
    except Exception as e:
...
def get_oauth_auth_url() -> dict:
    ...
    if not client_id:
        return {
            "ok": False,
            "error": "Sin client_id en _integrations/youtube_oauth.json.",
        }
...
def exchange_auth_code(code: str) -> dict:
    ...
    try:
        ...
        return {
            "ok": True,
            "message": "Autenticación completada. El sistema puede subir videos de forma autónoma.",
        }
    except Exception as e:
        return {"ok": False, "error": str(e)}
