...
def write_essay(topic_config: Dict) -> Dict[str, Any]:
    ...
    if not provider_names:
        logging.error(
            "[!] Ningún proveedor de IA está activo. No se puede generar el ensayo filosófico."
        )
    ...
    for res in sorted_results:
        ...
        try:
            ...
            try:
                ...
                logging.info(
                    f"[green]✓ Redacción filosófica exitosa (GANADOR) usando {provider_name} en {elapsed}s.[/]"
                )
                break
            except Exception:
                ...
                if brace:
                    try:
                        ...
                        logging.info(
                            f"[green]✓ JSON filosófico extraído por regex (GANADOR) con {provider_name} en {elapsed}s.[/]"
                        )
                        break
                    except Exception:
                        pass
    ...
    if not essay_data:
        raise RuntimeError(
            "El enjambre de modelos falló o se agotó. Abortando generación."
        )
    ...
    normalized = {
        ...
        "image": CATEGORY_IMAGE_MAP.get(
            translated_data.get("category", ""), CATEGORY_IMAGE_MAP["default"]
        ),
...
def update_essays_json(new_essay: Dict[str, Any]):
    ...
    try:
        ...
        topic_text = (
            f"Reflexión: {new_essay.get('title', '')}. {new_essay.get('excerpt', '')}"
        )
        ...
        logging.info(
            f"[green]✓ Video Vertical de Ensayo (TikTok) encolado automáticamente: {video_title}[/]"
        )
    except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--topic-index", type=int, default=None, help="Índice del tema (0-7)"
    )
    ...
    logging.info("=" * 70)
    logging.info(
        f"  Gravity AI Essayist V1.0 - Ejecución: {datetime.now().isoformat()}"
    )
    logging.info("=" * 70)
    ...
    if args.topic_index is not None and 0 <= args.topic_index < len(ESSAY_TOPICS):
        ...
        available_topics = [
            t for t in ESSAY_TOPICS if t["topic"] not in existing_titles
        ]
        if not available_topics:
            logging.info(
                "[*] Todos los temas base agotados. Reiniciando ciclo de temas."
            )
