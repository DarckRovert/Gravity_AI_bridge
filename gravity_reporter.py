...
def get_real_world_topic() -> str:
    ...
    try:
        ...
        logging.info("[*] Escaneando matriz RSS global...")
...
def write_article(search_results: str, prompt_override: str = None) -> Dict[str, Any]:
    ...
    user_prompt = (
        f"Aquí están los resultados de la búsqueda web de hoy:\n\n{search_results}\n\n"
    )
    if prompt_override:
    ...
    if not provider_names:
        logging.error(
            "[!] Ningún proveedor de IA está activo. No se puede generar el artículo."
        )
        return {}

    logging.info(
        f"[*] Lanzando petición en paralelo a múltiples IA: {', '.join(provider_names)}"
    )

    from core.multi_agent import compare

    # Usamos max_tokens prudente para soportar todos los modelos locales e IA en nube.
    results = compare(
        messages=messages,
        providers=provider_names,
        n_models=len(provider_names),
        options={"temperature": 0.6, "max_tokens": 2500},
        timeout=180.0,
    )

    article_data = None

    # Ordenar resultados por los más rápidos primero (Modo Carrera)
    ...
    for res in sorted_results:
        ...
        if (
            not response_raw
            or "[Error" in response_raw
            or "offline" in response_raw
            or "[No results]" in response_raw
        ):
            logging.warning(
                f"[!] {provider_name} ({model}) falló o dio error en {elapsed}s."
            )
            continue

        logging.info(
            f"\n[*] Evaluando respuesta de {provider_name} ({model}) completada en {elapsed}s"
        )
        ...
        try:
            ...
            try:
                ...
                logging.info(
                    f"[green]✓ Redacción exitosa (GANADOR) usando {provider_name} en {elapsed}s.[/]"
                )
                break  # Éxito, salir de la carrera
            except Exception as e:
                logging.warning(
                    f"[!] Error parseando JSON de {provider_name}: {e}. Intentando regex..."
                )
                ...
                if json_match:
                    try:
                        ...
                        logging.info(
                            f"[green]✓ JSON extraído por regex (GANADOR) con {provider_name} en {elapsed}s.[/]"
                        )
                        break
                    except Exception:
                        pass
                ...
                if repaired_str:
                    try:
                        ...
                        logging.info(
                            f"[green]✓ JSON reparado parcialmente (GANADOR) con {provider_name} en {elapsed}s.[/]"
                        )
                        break
                    except Exception:
                        pass
            ...
            logging.warning(
                f"[!] Error al limpiar la respuesta de {provider_name}: {e}"
            )
            continue

    if not article_data:
        logging.error(
            "[!] El enjambre multi-agencia falló o se agotó. Todos los modelos fallaron. Abortando redacción."
        )
        raise RuntimeError("El enjambre de modelos no devolvió un JSON válido.")
    ...
    if "excerpt" not in normalized:
        normalized["excerpt"] = (
            "Reporte interceptado de los nodos de Gravity AI en el subsuelo."
        )
    if "fullText" not in normalized:
        normalized["fullText"] = (
            "### Canal de contingencia activo\n\nNo se pudo decodificar el contenido completo."
        )
    ...
    try:
        logging.info(
            "[*] Pre-calentando imagen IA en Pollinations (esperando generación)..."
        )
        ...
        logging.warning(
            f"[!] Aviso: No se pudo pre-calentar la imagen. Se generará on-demand. {e}"
        )
...
def update_news_json(new_article: Dict[str, Any]):
    ...
    logging.info(
        f"[+] Artículo '{new_article.get('title', 'Sin Título')}' agregado con éxito a news.json"
    )
    ...
    try:
        ...
        logging.info(
            f"[green]✓ Video Vertical (TikTok) encolado automáticamente: {video_title}[/]"
        )
    except Exception as e:
...
def publish_changes():
    ...
    try:
        ...
        subprocess.run([sys.executable, sync_script], cwd=PORTAL_DIR, check=True)
    ...
    logging.info(
        "[*] Preparando publicación en GitHub para despliegue automático en Netlify..."
    )
    try:
        if not os.path.isdir(PORTAL_DIR):
            logging.warning(
                f"[!] El directorio {PORTAL_DIR} no existe. Omitiendo publicación."
            )
            return

        # Forzar configuración global de seguridad en Git
        subprocess.run(
            ["git", "config", "--global", "--add", "safe.directory", "*"], check=False
        )
        subprocess.run(["git", "status"], cwd=PORTAL_DIR, check=False)

        if not os.path.isdir(os.path.join(PORTAL_DIR, ".git")):
            logging.warning(
                f"[!] {PORTAL_DIR} no es un repositorio Git. Omitiendo publicación."
            )
            return
        ...
        # Prevenir bloqueos si Git pide credenciales
        env = os.environ.copy()
        ...
        if push_res.returncode == 0:
            logging.info(
                "[green]✓ Publicación exitosa. Netlify se actualizará en segundos.[/]"
            )
        else:
            logging.error(
                f"[!] Error al publicar en Netlify (Push fallido): {push_res.stderr.strip()}"
            )

    except subprocess.CalledProcessError as e:
        logging.error(f"[!] Error ejecutando comando local de Git: {e}")
    except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--topic", type=str, default=None, help="Tema de investigación específico"
    )
    parser.add_argument(
        "--focus", type=str, default=None, help="Enfoque particular de redacción"
    )
    ...
    logging.info(
        f"  Gravity AI Reporter V16.0 PRO - Ejecución: {datetime.now().isoformat()}"
    )
    logging.info(
        "======================================================================"
    )
