...
class TestMemoryAndRouting(unittest.TestCase):
    ...
    def test_task_routing_scoring(self):
        ...
        self.assertGreater(score_llava, score_nomic, "La tarea vision debe favorecer masivamente a llava sobre nomic")

        # 3. Verificar scoring de Embedding
        score_nomic_embed = _score_model(r, "nomic-embed-text-v1.5.f16.gguf", "embedding")
        score_llava_embed = _score_model(r, "llava-phi-3-mini-int4.gguf", "embedding")
        self.assertGreater(score_nomic_embed, score_llava_embed, "La tarea embedding debe favorecer masivamente a nomic sobre llava")

        # 4. Verificar penalización de nomic en chats comunes
        score_nomic_chat = _score_model(r, "nomic-embed-text-v1.5.f16.gguf", "code")
        self.assertLess(score_nomic_chat, 0.0, "Nomic debe estar penalizado severamente en chats comunes como code")
        ...
        self.assertGreater(score_qwen, score_hermes, "La tarea code debe favorecer a Qwen-Coder sobre Hermes")

    @patch('psutil.virtual_memory')
    @patch('os.path.getsize')
    @patch('llama_cpp.Llama')
    def test_proactive_memory_eviction(self, mock_llama, mock_getsize, mock_virtual_memory):
        # Configurar mocks
        ...
        self.assertNotIn("old_model.gguf", provider._instances, "El modelo antiguo debió ser desalojado para liberar RAM")
        self.assertIn("new_model.gguf", provider._instances, "El nuevo modelo debió ser cargado")
