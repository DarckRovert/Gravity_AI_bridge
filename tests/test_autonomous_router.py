...
def test_autonomous_router():
    ...
    if not health.is_healthy:
        print("[!] Native Llama no está healthy. Probablemente faltan modelos o llama-cpp-python.")
        return
        
    print(f"[OK] Native Llama detectado. Modelos encontrados: {[m['name'] for m in health.models]}")

    print("\n[+] Solicitando Tarea 1: 'bounty'")
    messages = [{"role": "user", "content": "Escribe una frase corta."}]
    
    import core.provider_manager
