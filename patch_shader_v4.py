...
vec2 map(vec3 p) {
    ...
    // Andenes y Terrazas (SAFE SDF loops)
    float terraces = 1e10;
    for(int i=0; i<8; i++) {
        float h = float(i) * 1.0;
        float z = float(i) * -1.5;
    ...
    // Habitaciones
    vec3 hp = p; hp.y -= 1.0; hp.x = abs(hp.x) - 7.0; 
    ...
    // CHACANA MÍSTICA (Cruz Andina Flotante y Giratoria)
    // Se ubica majestuosa en el cielo, flotando e interactuando
    vec3 cp = p - vec3(0.0, 10.0 + bass*0.5, -25.0);
    cp.xz *= rot(time * 0.2); // Rotación lenta
    cp.xy *= rot(sin(time*0.1)*0.2);
    
    // Construcción de los 3 escalones de la Cruz Andina
    float b1 = sdBox(cp, vec3(4.0, 1.5, 1.0)); // Brazo horizontal
    float b2 = sdBox(cp, vec3(1.5, 4.0, 1.0)); // Brazo vertical
    float b3 = sdBox(cp, vec3(2.5, 2.5, 1.0)); // Escalón intermedio
    float chacana = min(b1, min(b2, b3));
    
    // Hueco circular central para el sol
    float agujero = sdCylinder(cp.yxz, vec2(0.8, 1.5));
    chacana = max(chacana, -agujero);
    
    if(chacana < res.x) res = vec2(chacana, 4.0); // ID 4.0 = Chacana dorada
...
void main() {
    ...
    if (ct < 10.0) {
        ...
        target = vec3(0.0, 5.0 + t_fase*5.0, -15.0); // Mirando hacia la Chacana
    } else {
        float t_fase = (ct - 10.0) / 10.0;
        ro = vec3(0.0 + sin(t_fase*6.28)*3.0, 13.0 - t_fase*4.0, -10.0 - t_fase*10.0);
        target = vec3(0.0, 10.0, -25.0); // Mirando hacia la Chacana
    }
    ro.y += bass * 0.3;
    ...
    // Solsticio Intihuatana: Sol rojo-dorado peruano bajo
    ...
    // Cielo de Perú (Rojo Oscuro abajo, Dorado medio, Blanco/Azul arriba)
    ...
    if(t < tMax) {
        ...
        vec3 objCol = vec3(0.4, 0.4, 0.35); // Base andesita
        
        if (mat_id == 4.0) {
            // Material Místico para la Chacana (Oro Andino reflectante)
            objCol = vec3(1.0, 0.8, 0.3);
            dif += pow(clamp(dot(reflect(rd, n), sunDir), 0.0, 1.0), 32.0) * 2.0; // Especular
            amb += 0.5; // Brillo interno
        } else {
            objCol *= 0.8 + 0.4 * fbm(pos * 5.0); 
            // Musgo en la piedra regular
        ...
        vec3 lin = vec3(1.5, 1.0, 0.7) * dif * sha; // Luz cálida
        lin += vec3(0.2, 0.3, 0.4) * amb * ao; 
        
        col = objCol * lin;
        
        // Efecto LUMINISCENTE: LÍNEAS DE NAZCA sobre el suelo (mat_id 1.0)
        if (mat_id == 1.0) {
            float r = length(pos.xz);
            float ang = atan(pos.z, pos.x);
            // Dibujar un patrón geométrico/espiral estilo Nazca
            float spiral = abs(sin(r * 1.5 - ang * 4.0 + time));
            float lines = smoothstep(0.05, 0.02, spiral);
            lines *= smoothstep(40.0, 0.0, r); // Fade en la distancia
            
            // Emisión de luz mágica roja/blanca, reactiva a los bajos
    ...
    // Volumétrica teñida de oro/rojo
    col += vec3(1.0, 0.6, 0.3) * volumetric * 0.08;
    
    // Post-procesado (Tonemapping)
...
if not pattern.search(content):
    print("Error: Could not find INCA_MATH_FS V3 in file. The regex might be wrong.")
else:
    new_content = pattern.sub(NEW_SHADER, content)
    with open(filepath, "w", encoding="utf-8") as f:
        f.write(new_content)
    print("Patch V4 (Peru Edition) applied successfully.")
