"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  GRAVITY AI BRIDGE - INSTALADOR STANDALONE V16.0 PRO [Ecosistema Total]            ║
...
def phase_choose_directory(cli_dir: str = None, in_place: bool = False) -> str:
    ...
    if cli_dir:
        ...
        console.print(
            f"[dim]Directorio de instalación (argumento): [bold]{target}[/][/]"
        )
        return target

    if in_place:
        console.print(
            f"[dim]Instalación en sitio (modo desarrollo): [bold]{SOURCE_DIR}[/][/]"
        )
        return SOURCE_DIR
    ...
    while True:
        raw = Prompt.ask("[bold cyan]Ruta de instalación[/]", default=default).strip()
        ...
        try:
            ...
            if free_mb < 500:
                console.print(
                    f"[red]⚠ Solo {free_mb:.0f} MB libres en {drive}. Se necesitan mínimo 500 MB.[/]"
                )
                continue
        except Exception:
            pass
        ...
        if os.path.exists(target) and os.listdir(target):
            has_manifest = os.path.exists(
                os.path.join(target, "_install_manifest.json")
            )
            if has_manifest:
                console.print(
                    f"[yellow]  Instalación previa detectada en {target}. Se actualizará.[/]"
                )
            else:
                console.print("[yellow]  El directorio ya existe y no está vacío.[/]")
...
def phase_components() -> dict:
    ...
    for i, (cid, meta) in enumerate(COMPONENTS.items(), 1):
        selection[cid] = Confirm.ask(
            f"  [{i}] ¿Instalar [cyan]{meta['label']}[/]?", default=meta["default"]
        )
...
def _generate_config(target_dir: str, hw: dict, engines: dict):
    ...
    if engines:
        default_prov = (
            ...
            else list(engines.keys())[0].lower().replace(" ", "_")
        )
    else:
        default_prov = "auto"
...
def main():
    ...
    if not Confirm.ask(
        "[bold cyan]¿Iniciar la instalación de Gravity AI Bridge?[/]", default=True
    ):
    ...
    if sys.version_info < (3, 10):
        console.print(
            f"[red]✖ Python 3.10+ requerido. Versión actual: {sys.version.split()[0]}[/]"
        )
    ...
    with Progress(
        ...
        if os.path.abspath(target_dir) != os.path.abspath(SOURCE_DIR):
            ...
            t_skip = progress.add_task(
                "[dim]Instalación in-place (sin copia)...", total=1
            )
        ...
        t10 = progress.add_task(
            "[cyan]Escribiendo manifiesto de instalación...", total=1
        )
