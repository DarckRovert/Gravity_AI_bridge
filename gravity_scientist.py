...
def write_science_article(search_results: str, query: str) -> Dict[str, Any]:
    ...
    if not provider_names:
        logging.error(
            "[!] Ningún proveedor de IA está activo. No se puede generar el artículo científico."
        )
    ...
    for res in sorted_results:
        ...
        try:
            ...
            try:
                ...
                logging.info(
                    f"[green]✓ Redacción científica exitosa (GANADOR) usando {provider_name} en {elapsed}s.[/]"
                )
                break
            except Exception:
                ...
                if brace:
                    try:
                        ...
                        logging.info(
                            f"[green]✓ JSON científico extraído por regex (GANADOR) con {provider_name} en {elapsed}s.[/]"
                        )
                        break
                    except Exception:
                        pass
    ...
    try:
        logging.info("[*] Pre-calentando imagen IA científica...")
...
def update_science_json(new_article: Dict[str, Any]):
    ...
    try:
        ...
        logging.info(
            f"[green]✓ Video Vertical Científico (TikTok) encolado automáticamente: {video_title}[/]"
        )
    except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--query", type=str, default=None, help="Query de búsqueda específica"
    )
    ...
    logging.info("=" * 70)
    logging.info(
        f"  Gravity AI Scientist V1.0 - Ejecución: {datetime.now().isoformat()}"
    )
    logging.info("=" * 70)
