...
try:
    ...
    checks["LLM Provider"] = (
        f"OK - {best_p.name}/{best_m}" if best_p else "FALLO - Sin proveedor"
    )
...
try:
    ...
    total = conn.execute("SELECT COUNT(*) FROM video_jobs").fetchone()[0]
        ...
        "SELECT COUNT(*) FROM video_jobs WHERE status='done'"
    ).fetchone()[0]
    uploaded = conn.execute(
        "SELECT COUNT(*) FROM video_jobs WHERE upload_status='uploaded'"
    ).fetchone()[0]
    checks["Video Queue DB"] = (
        f"OK - {total} jobs, {done} done, {uploaded} subidos a YT"
    )
    conn.close()
except Exception as e:
...
try:
    ...
    checks["Monetizacion Modulos"] = "OK - " + ", ".join(
...
try:
    ...
    checks["Agentic ToolEngine"] = (
        f"OK - {len(engine.tools)} tools: {list(engine.tools.keys())}"
    )
except Exception as e:
...
try:
    ...
    links = aff.get("links", {})
    checks["Affiliates Links"] = (
        f"OK - {len(links)} nichos configurados: {list(links.keys())}"
    )
except Exception as e:
...
for plat in ["tiktok", "instagram"]:
    ...
    if os.path.isfile(path):
        ...
        checks[f"{plat.title()} Creds"] = (
            "OK - credenciales presentes" if has_key else "PENDIENTE - archivo vacío"
        )
    else:
...
print("=== AUDITORIA SISTEMA GRAVITY V16.0 ===")
