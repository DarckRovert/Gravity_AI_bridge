        ...
        for root, _, files in os.walk(d):
            for file in files:
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=15) as executor:
                    future_to_index = {executor.submit(process_paragraph, p): i for i, p in enumerate(paragraphs)}
                    completed = 0
                    for future in concurrent.futures.as_completed(future_to_index):
                        idx = future_to_index[future]
                        new_paragraphs[idx] = future.result()
                        completed += 1
                        print(f"  -> Procesando párrafo {completed}/{len(paragraphs)} (Turbo)", end="\r")
...
if __name__ == "__main__":
    ...
    import subprocess
    try:
        script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "limpiador_textos.py")
    ...
    os.makedirs(target_base, exist_ok=True) # Aseguramos que la carpeta exista aunque el usuario la haya borrado por completo
