"""
╔══════════════════════════════════════════════════════════════════════════════╗
║      GRAVITY AI BRIDGE - AUDITOR SENIOR V16.0 PRO [Ecosistema Total]               ║
...
class SettingsManager:
    """Surgical wrapper for ConfigManager V16.0 PRO compatibility."""
    ...
    def save(self):
        ...
        config.config["model"]["temperature"] = self.data["advanced_params"][
            "temperature"
        ]
        config.save()
    ...
    @property
    def mode(self):
        return self.data.get("mode", "production")
...
def first_run_check():
    ...
    _console.print(
        Panel(
            Align.center(
                "[bold white]Bienvenido a Gravity AI Bridge V16.0 PRO[/]\n"
    ...
    if engines_found:
        _console.print(
            f"[green]✓ Motores locales detectados:[/] {', '.join(engines_found.keys())}"
        )
    else:
        _console.print("[yellow]⚠  No se detectaron motores de IA locales.[/]")
        _console.print(
            "[dim]  Instala Ollama (ollama.ai) para usar modelos locales, o configura un proveedor cloud.[/]"
        )
    ...
    if not engines_found:
        if Confirm.ask(
            "[cyan]¿Deseas configurar una API Key de proveedor cloud ahora?[/]",
            default=True,
        ):
            ...
            if key.strip():
                ...
                _console.print("[green]✓ Clave guardada cifrada.[/]")
        _console.print()

    # ── Tutorial rápido ───────────────────────────────────────────────────────
    t = Table(
        title="[bold cyan]Comandos esenciales para empezar[/]",
        box=box.SIMPLE_HEAD,
        padding=(0, 2),
    )
    ...
    _console.print()
    _console.print(
        "[dim]Este mensaje solo aparece la primera vez. Escribe [bold]/help[/] en cualquier momento.[/]"
    )
    _console.print()
...
class AuditorCLI:
    ...
    def _load_system_prompt(self, kb_data: dict = None) -> str:
        ...
        if mode == "Omni-Audit":
            base += (
                "\nMODO: Omni-Audit (V16.0 PRO Premium). "
        ...
        try:
            ...
            if kb_data is None and os.path.exists(KNOWLEDGE_FILE):
                ...
                bad = [
                    ...
                    if any(
                        k in x.lower()
                        for k in ("corrupto", "inválido", "eliminad", "backup")
                    )
                ]
                if bad:
                    for warn in bad:
...
» Modo:  [dim]{hw['gpu_type'].upper()}[/]"""
    ...
    def cmd_help(self):
        ...
        t.add_row(
            "/model", "Picker interactivo para cambiar modelo/proveedor al vuelo."
        )
        t.add_row("/mode", "Cambiar modo: production / development / Omni-Audit.")
        t.add_row(
            "/providers", "Estado y latencia de todos los motores locales y cloud."
        )
        t.add_row(
            "/keys list|set|del", "Gestiona claves API de proveedores cloud (cifradas)."
        )
        t.add_row("/rag <búsqueda>", "Busca en el índice de documentos local.")
        t.add_row(
            "/search <búsqueda>", "Búsqueda web en vivo (inyecta contexto al prompt)."
        )
        ...
        t.add_row(
            "/verify <archivo>", "Audita un archivo con el Agente de Verificación."
        )
        t.add_row("/mcp <ruta>", "Conecta con un servidor MCP externo (stdio).")
        t.add_row(
            "/cache [on|off|clear]",
            "Gestiona la caché de respuestas LLM (por defecto OFF).",
        )
        ...
        console.print(
            Panel(
                t,
                title="[bold cyan]Comandos Disponibles - Gravity AI V16.0 PRO[/]",
                border_style="blue",
            )
        )
    ...
    def cmd_keys(self, args: str):
        ...
        if subcmd == "list":
            ...
            console.print(
                "[red]Uso: /keys list | /keys set [proveedor] | /keys del <proveedor>[/]"
            )
        ...
        try:
            ...
            if w_load:
                bad = [
                    x
                    for x in w_load
                    if "eliminad" in x.lower() or "backup" in x.lower()
                ]
                for warn in bad:
            ...
            if stripped_new in existing_stripped:
                console.print(
                    "[yellow]Regla ya existe en el knowledge base (duplicado ignorado).[/]"
                )
                return
            ...
            if ok:
                console.print(
                    f"[green]✓ Regla persistida en _knowledge.json[/] [dim]({len(rules)} total)[/]"
                )
            else:
                for w in w_save:
    ...
    def cmd_verify(self, file_path: str):
        ...
        try:
            ...
            score_color = (
                "green"
                if result["score"] >= 80
                else ("yellow" if result["score"] >= 50 else "red")
            )
            console.print(
            ...
            if result["findings"]:
                for finding in result["findings"]:
                    ...
                    clr = (
                        "red"
                        if lvl == "CRITICAL"
                        else ("yellow" if lvl == "WARNING" else "dim")
                    )
    ...
    def _mcp_session(self, server_path: str):
        ...
        if tools_available:
            console.print(
                f"[green]✓ Conectado. {len(tools_available)} herramientas disponibles:[/]"
            )
            for tool in tools_available:
                console.print(
                    f"  [cyan]{tool.get('name', '?')}[/] - {tool.get('description', '')}"
                )
        else:
            console.print("[yellow]Conectado pero sin herramientas listadas.[/]")
        console.print(
            "[dim]Sesión MCP activa. Escribe '/mcp-exit' para desconectar.[/]"
        )
        try:
            while True:
    ...
    def process_command(self, user_input: str) -> bool:
        ...
        if cmd in ("exit", "quit", "/exit", "/quit"):
            ...
            console.print(
                f"[green]✓ Purga completa. ({saved} tokens de razonamiento eliminados permanentemente).[/]"
            )
        ...
        if cmd == "/cost":
            console.print(
                Panel(CostTracker.summary_text(), title="[yellow]Consumo API[/]")
            )
            return True
        ...
        if cmd == "/branch":
            ...
            console.print(
                f"[green]✓ Fork creado: {real}. (Sesión actual sigue intacta).[/]"
            )
            return True
        ...
        if cmd == "/load":
            if not args:
                console.print(
                    "[red]Especifica el nombre de sesión. Usa /sessions para listar.[/]"
                )
                return True
            if self.session.load(args):
                console.print(
                    f"[green]✓ Sesión '{args}' cargada. ({len(self.history)} mensajes)[/]"
                )
            else:
        ...
        if cmd == "/index":
            ...
            if not os.path.exists(args):
                ...
                return True
            num = (
                RAGIndexer.index_file(args)
                if os.path.isfile(args)
                else RAGIndexer.index_folder(args)[1]
            )
        ...
        if cmd == "/search":
            ...
            self.history.append(
                {"role": "user", "content": f"Contexto Web Inyectado:\n{out}"}
            )
            return True
        ...
        if cmd == "/cache":
            ...
            if sub == "on":
                ...
                CacheEngine.disable()
                console.print(
                    "[yellow]✓ Caché LLM deshabilitada (siempre generará respuestas nuevas).[/]"
                )
            elif sub == "clear":
                cnt = CacheEngine.clear()
                console.print(
                    f"[green]✓ Caché purgada en SQLite ({cnt} registros eliminados).[/]"
                )
            else:
                st = CacheEngine.stats()
                enabled_str = "[green]ON[/]" if st["enabled"] else "[yellow]OFF[/]"
                console.print(
                    f"Estado de Caché: {enabled_str} | Registros: {st['entries']} ({st['size_kb']} KB) | Hit Rate: {st['hit_rate']}"
                )
            return True
    ...
    def interactive_loop(self):
        ...
        while True:
            try:
                ...
                if not tgt_p:
                    console.print(
                        "[red]ERROR: Ningún motor de IA disponible. Inicia uno local o añade una Key con /keys set.[/]"
                    )
                    self.history.pop()
                    continue
                ...
                # V16.0 PRO: Inyectar MemDir (conocimiento de directorio) antes de buscar RAG
                ...
                if (
                    "busca en" in user_msg.lower()
                    or "del contexto" in user_msg.lower()
                    or "en mi codigo" in user_msg.lower()
                ):
                ...
                if cached:
                    ...
                    t0 = time.time()
                    try:
                        for chunk in provider_manager.stream(
                            self.history, tgt_m, tgt_p, opts
                        ):
                            if chunk:
                                if ttft == 0:
                ...
                for tname, succ, out in t_runs:
                    ...
                    console.print(
                        Panel(
                            ...
                            title=f"[{color}]Tool: {tname}[/]",
                            border_style=color,
                        )
                    )
                    self.history.append(
                        {"role": "user", "content": f"Output de {tname}:\n{out[:2000]}"}
                    )

                # Métricas de sesión (footer compacto, sin redibujar banner)
                in_tok = len(user_msg) // 4
                out_tok = len(final_text) // 4
                cost = CostTracker.estimate(
                    tgt_p, tgt_m, len(user_msg), len(final_text)
                )
                ...
                console.print(
                    f"[dim]({elapsed:.1f}s | {tps:.0f} tok/s | {ttft_str}${cost:.5f})[/]"
                )
                ...
                if trimmed > 0:
                    console.print(
                        f"[dim](Contexto optimizado: {trimmed} mensajes antiguos descartados.)[/]"
                    )
...
if __name__ == "__main__":
    ...
    _parser = _ap.ArgumentParser(
        prog="ask_deepseek",
        description="Gravity AI Bridge V16.0 PRO CLI",
        add_help=False,
    )
    ...
    if session_id:
        console.print(
            f"[green]Iniciando sesión aislada: {session_id} (Role: {role})[/]"
        )
    ...
    resp = provider_manager.complete(
        cli.history, bm, bp, cli.sm.data.get("advanced_params", {})
    )
    print(resp)
