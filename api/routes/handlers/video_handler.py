...
def handle_video_voices(handler):
    try:
        ...
        try:
            import sys as _sys
            BASE_DIR_g = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
...
def handle_video_engines(handler):
    try:
        ...
        def _check_fooocus() -> bool:
            try:
                import sys as _sys
                BASE_DIR_f = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
        ...
        def _check_gemini() -> bool:
            try:
                import sys as _sys2
                BASE_DIR_gt = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
...
def handle_video_create(handler):
    try:
        ...
        if not topic:
            ...
            handler._send_cors()
            handler.end_headers()
            handler.wfile.write(json.dumps({"error": "Campo 'topic' requerido."}).encode())
            return
        ...
        _def_anim = 1
        try:
            _bdir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
...
def handle_course_generate(handler):
    try:
        ...
        if not title:
            ...
            handler._send_cors()
            handler.end_headers()
            handler.wfile.write(json.dumps({"error": "Campo 'title' requerido."}).encode())
            return
