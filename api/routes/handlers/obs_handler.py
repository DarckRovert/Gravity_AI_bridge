...
def handle_obs_scenes(handler):
    ...
    try:
        ...
        body = json.dumps(
            {"scenes": scenes, "current_scene": current}, indent=2
        ).encode("utf-8")
        handler.send_response(200)
        handler.send_header("Content-Type", "application/json")
        handler._send_cors()
...
def handle_obs_scene_items(handler):
    ...
    try:
        ...
        body = json.dumps(
            {"scene_name": scene_name, "items": items, "count": len(items)}, indent=2
        ).encode("utf-8")
        handler.send_response(200)
        handler.send_header("Content-Type", "application/json")
        handler._send_cors()
...
def handle_obs_inputs(handler):
    ...
    try:
        ...
        body = json.dumps({"inputs": inputs, "count": len(inputs)}, indent=2).encode(
            "utf-8"
        )
        handler.send_response(200)
        handler.send_header("Content-Type", "application/json")
...
def handle_obs_overlays(handler):
    ...
    try:
        ...
        body = json.dumps(
            {"overlays": overlays, "count": len(overlays)}, indent=2
        ).encode("utf-8")
        handler.send_response(200)
        handler.send_header("Content-Type", "application/json")
        handler._send_cors()
...
def handle_obs_scene_switch(handler):
    ...
    try:
        ...
        if not scene_name:
            ...
            handler._send_cors()
            handler.end_headers()
            handler.wfile.write(json.dumps({"error": "scene_name requerido"}).encode())
            return
        from core.obs_client import get_client
...
def handle_obs_source_toggle(handler):
    ...
    try:
        ...
        if not scene_name or not scene_item_id:
            ...
            handler.end_headers()
            handler.wfile.write(
                json.dumps({"error": "scene_name y scene_item_id requeridos"}).encode()
            )
            return
        from core.obs_client import get_client
...
def handle_obs_audio_mute(handler):
    ...
    try:
        ...
        if not input_name:
            ...
            handler._send_cors()
            handler.end_headers()
            handler.wfile.write(json.dumps({"error": "input_name requerido"}).encode())
            return
        from core.obs_client import get_client
...
def handle_obs_spark_generate(handler):
    ...
    try:
        ...
        if not prompt:
            ...
            handler.end_headers()
            handler.wfile.write(
                json.dumps({"error": "Campo 'prompt' requerido"}).encode()
            )
            return
        ...
        log.warning(
            f"[GravitySpark] Client aborted connection during overlay generation: {e}"
        )
    except Exception as e:
        log.error(
            f"[GravitySpark] /v1/obs/spark/generate error: {traceback.format_exc()}"
        )
        try:
            handler.send_response(500)
            handler._send_cors()
...
def handle_obs_spark_edit(handler):
    ...
    try:
        ...
        if not overlay_id or not new_prompt:
            ...
            handler.end_headers()
            handler.wfile.write(
                json.dumps({"error": "overlay_id y prompt requeridos"}).encode()
            )
            return
...
def handle_obs_spark_remove(handler):
    ...
    try:
        ...
        if not overlay_id:
            ...
            handler._send_cors()
            handler.end_headers()
            handler.wfile.write(json.dumps({"error": "overlay_id requerido"}).encode())
            return
