...
def handle_v2v_start(req):
    try:
        ...
        if is_running:
            body = json.dumps(
                {"ok": False, "message": "El motor V2V ya está en ejecución."}
            ).encode()
        ...
        if not os.path.exists(bat_path):
            body = json.dumps(
                {"ok": False, "message": "run_v2v.bat no encontrado."}
            ).encode()
...
def handle_v2v_stop(req):
    try:
        ...
        body = json.dumps(
            {
                "ok": True,
                "message": f"Motor V2V detenido. Procesos terminados: {killed_count}",
            }
        ).encode()
        req.send_response(200)
        req.send_header("Content-Type", "application/json")
