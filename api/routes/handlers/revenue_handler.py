...
def handle_youtube_auth_exchange(handler):
    try:
        ...
        if not code:
            ...
            handler.end_headers()
            handler.wfile.write(
                json.dumps({"error": "Campo 'code' requerido"}).encode()
            )
            return
...
def handle_youtube_upload(handler):
    try:
        ...
        row = conn.execute(
            "SELECT output_path, title, topic, thumbnail_path FROM video_jobs WHERE id=?",
            (job_id,),
        ).fetchone()
        conn.close()
        if not row or not row["output_path"]:
            ...
            handler.wfile.write(
                json.dumps(
                    {"error": f"Job #{job_id} no encontrado o sin video."}
                ).encode()
            )
            return
        ...
        body = json.dumps(
            {"ok": True, "job_id": job_id, "message": "Upload iniciado en background."}
        ).encode()
        handler.send_response(200)
        handler.send_header("Content-Type", "application/json")
        handler._send_cors()
