...
class PostRoutesMixin(PostChatMixin, PostMediaMixin, PostGameserverMixin, PostAgentMixin, PostSystemMixin):
    """
    ...
    def do_POST(self):
        ...
        if self._handle_post_chat(): return
        if self._handle_post_media(): return
        if self._handle_post_gameserver(): return
        if self._handle_post_agent(): return
        if self._handle_post_system(): return
