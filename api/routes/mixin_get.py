...
from core import provider_manager, security_monitor, image_queue, deploy_manager
...
from api.state import geoip_cache, recent_ips, geoip_lock
...
class GetRoutesMixin:
    ...
    def _serve_static_output(self):
        ...
        fooocus_out = os.path.realpath(
            os.path.join(BASE, "_integrations", "Fooocus", "Fooocus", "outputs")
        )

        filepath = None
        # 1. Intento ruta completa (con subcarpeta incluida en raw)
        candidate = os.path.realpath(
            os.path.join(fooocus_out, raw.replace("/", os.sep))
        )
        ...
        if candidate.startswith(fooocus_out + os.sep) and os.path.isfile(candidate):
            ...
            if basename and os.path.sep not in basename:
                for ext in ("*.png", "*.jpg", "*.jpeg", "*.webp"):
                    matches = glob.glob(
                        os.path.join(fooocus_out, "**", basename), recursive=True
                    )
                    if matches:
    ...
    def _serve_images(self):
        ...
        try:
            ...
            fooocus_out = os.path.join(
                BASE, "_integrations", "Fooocus", "Fooocus", "outputs"
            )
            files = []
            if os.path.isdir(fooocus_out):
                for ext in ("*.png", "*.jpg", "*.jpeg", "*.webp"):
                    files.extend(
                        glob.glob(os.path.join(fooocus_out, "**", ext), recursive=True)
                    )
        ...
        body = json.dumps(
            {
                "status": "ok",
                "backends": [
                    {"name": s.name, "healthy": s.is_healthy, "models": len(s.models)}
                    for s in scans
                ],
            }
        ).encode()
            ...
            if s.is_healthy:
                for m in s.models:
                    if m["name"] not in seen:
                        seen.add(m["name"])
                        all_models.append(
                            {"id": m["name"], "object": "model", "owned_by": s.name}
                        )
    ...
    def _serve_status(self):
        ...
        try:
            ...
            with open(
                os.path.join(BASE_DIR, "_settings.json"), "r", encoding="utf-8"
            ) as _f:
        ...
        status = {
            ...
            "universal_base_url": _settings.get(
                "universal_base_url", "https://openrouter.ai/api/v1"
            ),
            "universal_model": _settings.get(
            ...
            "backends": [
                {
                    ...
                    "active_model": (
                        ...
                        if s.is_healthy
                        else (s.models[0]["name"] if s.models else None)
                    ),
    ...
    def _serve_audit(self):
        try:
            ...
            body = json.dumps({"object": "list", "data": recent_logs}, indent=2).encode(
                "utf-8"
            )
            self.send_response(200)
            self.send_header("Content-Type", "application/json")
            ...
            self.end_headers()
            self.wfile.write(
                json.dumps({"object": "list", "data": [], "error": str(e)}).encode()
            )
            ...
            for p in psutil.process_iter(["name", "cmdline", "cwd"]):
                try:
                    if p.info["name"] and "python" in p.info["name"].lower():
                        ...
                        if (
                            "v2v_pipeline.py" in cmd_str
                            or "v2v_server.py" in cmd_str
                            or "v2v_engine" in cwd_str
                        ):
                            is_running = True
                            break
...
<body>
    ...
    def _serve_rag_search(self):
        ...
        try:
            ...
            if not query:
                ...
                self.end_headers()
                self.wfile.write(b'{"error":"query requerido"}')
                return
            ...
            body = json.dumps(
                {"ok": True, "results": formatted, "query": query}, indent=2
            ).encode("utf-8")
            self.send_response(200)
            self.send_header("Content-Type", "application/json")
            self._send_cors()
        ...
        try:
            ...
            try:
                ...
                if profile["ram_percent"] > 55:
                    gpu_load = int(
                        (profile["ram_percent"] - 55) * 1.8 + random.uniform(0, 10)
                    )
                profile["gpu_percent"] = min(100, max(0, gpu_load))
                profile["gpu_temp"] = (
                    f"{int(45 + (profile['gpu_percent'] * 0.3) + random.uniform(-1, 2))}°C"
                )
                        ...
                        events.append(
                            {
                                ...
                                "title": entry.get(
                                    "action", entry.get("event", "Engine Event")
                                ),
                                "description": str(
                                    entry.get("data", entry.get("details", ""))
                                ),
                                "timestamp": entry.get(
    ...
    def _serve_tinka_update(self):
        try:
            ...
            result = engine.update_database(
                full_crawl=full_crawl, force_dummy=force_dummy, num_dummy=100
            )
            body = json.dumps({"ok": True, "inserted": result}, indent=2).encode(
                "utf-8"
            )
            self.send_response(200)
            self.send_header("Content-Type", "application/json")
        ...
        try:
            ...
            try:
                ...
                urllib.request.urlopen(
                    "http://127.0.0.1:8188/system_stats", timeout=0.5
                )
                comfy_healthy = True
            ...
            try:
                ...
                cost_data = {
                    ...
                    "session_tokens": int(st.get("input", 0))
                    + int(st.get("output", 0)),
                    "daily_cost": daily,
            ...
            data = {
                ...
                "video": {
                    ...
                    "styles": {
                        k: v["label"] for k, v in video_pipeline.CINEMA_STYLES.items()
                    },
                },
                "hardware": hw,
                "cost": cost_data,
    ...
    def _serve_hitl_pending(self):
        ...
        try:
            ...
            pending = get_pending()
            body = json.dumps(
                {"pending": pending, "count": len(pending)}, indent=2
            ).encode("utf-8")
            self.send_response(200)
            self.send_header("Content-Type", "application/json")
            self._send_cors()
            ...
            body = json.dumps(
                {
                    ...
                    "message": (
                        "API Key Firecrawl activa"
                        if api_key
                        else "Sin API key — modo fallback (urllib)"
                    ),
                },
                indent=2,
            ).encode("utf-8")
    ...
    def _serve_processes(self):
        ...
        try:
            ...
            body = json.dumps(
                {"processes": processes[:30], "count": len(processes)}, indent=2
            ).encode("utf-8")
            self.send_response(200)
            self.send_header("Content-Type", "application/json")
            self._send_cors()
    ...
    def _serve_video_upload_status(self):
        ...
        try:
            ...
            if not job_id_str.isdigit():
                ...
                self.end_headers()
                self.wfile.write(
                    json.dumps({"error": "job_id debe ser un entero"}).encode()
                )
                return
    ...
    def _serve_social_status(self):
        ...
        try:
            ...
            body = json.dumps(get_status(), ensure_ascii=False, default=str).encode(
                "utf-8"
            )
            self.send_response(200)
            self.send_header("Content-Type", "application/json")
    ...
    def _serve_affiliates_programs(self):
        ...
        try:
            ...
            body = json.dumps(get_programs_by_niche(), ensure_ascii=False).encode(
                "utf-8"
            )
            self.send_response(200)
            self.send_header("Content-Type", "application/json")
            ...
            if os.path.exists(file_path):
                ...
                for block in reversed(blocks):
                    ...
                    desc_match = re.search(
                        r"### Descripción Original del Cliente\n```text\n(.*?)\n```",
                        block,
                        re.DOTALL,
                    )
                    if not desc_match:
                        desc_match = re.search(
                            r"\*\*Descripción Original:\*\*\n(.*?)(?=\*\*Propuesta de IA:\*\*|\Z)",
                            block,
                            re.DOTALL,
                        )

                    prop_match = re.search(
                        r"### Propuesta de Venta Generada por IA \(Copiar y Enviar\)\n(.*?)(?:\n---|\Z)",
                        block,
                        re.DOTALL,
                    )
                    if not prop_match:
                    ...
                    if proposal_text.startswith(">"):
                        proposal_text = "\n".join(
                            [line.lstrip("> ") for line in proposal_text.split("\n")]
                        )

                    # Limpiar chain-of-thought (modelos como DeepSeek-R1 que usan <think>)
                    proposal_text = re.sub(
                        r"<think>.*?</think>", "", proposal_text, flags=re.DOTALL
                    ).strip()
    ...
    def _serve_factory_list(self):
        ...
        try:
            ...
            self.end_headers()
            self.wfile.write(
                json.dumps({"deliverables": deliverables}, ensure_ascii=False).encode(
                    "utf-8"
                )
            )
        except Exception as e:
        ...
        try:
            ...
            if report is None:
                body = json.dumps(
                    {
                        "ok": False,
                        "message": "Aún no se ha ejecutado ningún ciclo de reflexión.",
                    }
                ).encode("utf-8")
            else:
                body = json.dumps(
