    ...
    def _handle_post_agent(self):
        if self.path == "/v1/bounties/action":
            try:
                ...
                if not url or not action:
                    ...
                    self.end_headers()
                    self.wfile.write(
                        json.dumps({"error": "url y action son requeridos"}).encode()
                    )
                    return True
        ...
        if self.path == "/v1/infiltrator/queue_bid":
            try:
                ...
                ok, msg = infiltrator.manager.queue_task(
                    {"type": "freelancer_bid", "url": url, "proposal": proposal}
                )
        ...
        if self.path == "/v1/factory/generate":
            try:
                ...
                if not plugin:
                    raise Exception(
                        f"No hay proveedor de IA disponible ({provider_name})."
                    )
                ...
                if "<think>" in response_text:
                    response_text = _re.sub(
                        r"<think>.*", "", response_text, flags=_re.DOTALL
                    ).strip()
                ...
                folder_name = "entregable_" + _datetime.datetime.now().strftime(
                    "%Y%m%d_%H%M%S"
                )
                ...
                with open(
                    _os.path.join(project_path, "RAW_RESPONSE.md"),
                    "w",
                    encoding="utf-8",
                ) as f:
        ...
        if self.path == "/v1/rag/ingest":
            try:
                ...
                try:
                    ...
                    t = threading.Thread(
                        target=ingest_directory, args=(sources_dir,), daemon=True
                    )
                    t.start()
                except Exception:
                    pass
                body_resp = json.dumps(
                    {
                        "ok": True,
                        "indexed": indexed,
                        "message": f"{indexed} archivo(s) guardados. Re-indexación iniciada.",
                    }
                ).encode("utf-8")
                self.send_response(200)
                self.send_header("Content-Type", "application/json")
        ...
        if self.path == "/v1/rag/toggle":
            try:
                ...
                with open(settings_path, "w", encoding="utf-8") as f:
                    json.dump(settings, f, indent=4, ensure_ascii=False)
                body = json.dumps(
                    {"ok": True, "rag_enabled": settings["rag_enabled"]}
                ).encode()
                self.send_response(200)
                self.send_header("Content-Type", "application/json")
                self._send_cors()
        ...
        if self.path == "/v1/journalist/portal/start":
            from api.routes.handlers.journalist_handler import (
                handle_journalist_portal_start,
            )
                    ...
                    results = [
                        {
                            "title": "Resultado",
                            "url": "",
                            "snippet": r.stdout or "Sin resultados",
                        }
                    ]
                body = json.dumps(
                    {"ok": r.success, "results": results, "query": query}
                ).encode("utf-8")
                self.send_response(200)
                self.send_header("Content-Type", "application/json")
                self._send_cors()
        ...
        if self.path == "/v1/tools/git":
            try:
                ...
                cwd = data.get("cwd", "").strip() or os.path.dirname(
                    os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
                )
                BASE = os.path.dirname(
                    os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        ...
        if self.path == "/v1/tools/grep":
            try:
                ...
                if not pattern:
                    ...
                    self.end_headers()
                    self.wfile.write(
                        json.dumps({"error": "pattern requerido"}).encode()
                    )
                    return True
                BASE = os.path.dirname(
                    os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
                    ...
                    body = json.dumps(
                        {
                            ...
                            "matches": [],
                            "count": 0,
                            "raw": "Timeout: búsqueda excedió 15s. Usa el parámetro 'path' para limitar el scope.",
                        }
                    ).encode("utf-8")
                    self.send_response(200)
                    self.send_header("Content-Type", "application/json")
                ...
                body = json.dumps(
                    {
                        "ok": ok,
                        "approval_id": aid,
                        "message": "Aprobado" if ok else "ID no encontrado",
                    }
                ).encode()
                self.send_response(200)
                self.send_header("Content-Type", "application/json")
                ...
                body = json.dumps(
                    {
                        "ok": ok,
                        "approval_id": aid,
                        "message": "Rechazado" if ok else "ID no encontrado",
                    }
                ).encode()
                self.send_response(200)
                self.send_header("Content-Type", "application/json")
        ...
        if self.path == "/v1/reflection/trigger":
            try:
                ...
                t = threading.Thread(
                    target=run_reflection_cycle, daemon=True, name="ReflectionTrigger"
                )
                t.start()
                body = json.dumps(
                    {
                        "ok": True,
                        "message": "Ciclo de auto-reflexión iniciado en background.",
                    }
                ).encode()
                self.send_response(200)
                self.send_header("Content-Type", "application/json")
        ...
        if self.path.startswith("/v1/reflection/patches/") and self.path.endswith(
            "/approve"
        ):
            try:
                patch_id = (
                    self.path.split("/v1/reflection/patches/")[1]
                    .replace("/approve", "")
                    .strip()
                )
