...
# ─── GET ENDPOINTS ────────────────────────────────────────────────
GET_ENDPOINTS = [
    "/", "/dashboard", "/health", "/v1/models", "/v1/status",
    "/v1/audit", "/v1/fooocus/status", "/v1/images", "/metrics",
    "/v1/security", "/v1/security/geoip", "/v1/queue", "/v1/deploy/status",
    "/v1/gameserver/status", "/v1/gameserver/log", "/v1/gameserver/players",
    "/v1/hardware", "/v1/hardware/stats", "/v1/cost", "/v1/watchdog",
    "/v1/sessions", "/v1/sessions/active", "/v1/rag/status",
    "/v1/fabricaweb/status", "/v1/video/status", "/v1/video/voices",
    "/v1/video/engines", "/v1/video/list", "/v1/video/animations",
    "/v1/image/health", "/v1/image/lab/history",
    "/v1/mcp/status", "/v1/hitl/pending", "/v1/tools/firecrawl/health",
    "/v1/gravity/context", "/v1/processes",
    "/v1/scheduler/status", "/v1/scheduler/niches",
    "/v1/youtube/status", "/v1/youtube/quota", "/v1/youtube/auth/url",
    "/v1/revenue/summary", "/v1/revenue/timeline", "/v1/revenue/top",
    "/v1/social/status", "/v1/affiliates/status", "/v1/affiliates/programs",
    "/v1/language/status", "/v1/v2v/status", "/v1/obs/status",
]
...
POST_ENDPOINTS = [
    # ── Chat / LLM ─────────────────────────────────────────────────
    ("/v1/chat/completions", {"messages": [{"role": "user", "content": "ping"}], "max_tokens": 5}, False, "Chat Completions (LLM)"),
    ("/v1/gravity/chat",     {"messages": [{"role": "user", "content": "ping"}]}, False, "Gravity Chat (RAG+LLM)"),
    ...
    ("/v1/tools/run",             {"code": "print('test')", "language": "python"}, True, "Tool: Code Run"),
    ("/v1/tools/grep",            {"pattern": "def run_server"}, False, "Tool: Grep (Timeout Expected)"),
    ("/v1/tools/scrape",          {"url": "http://example.com"}, True, "Tool: Scrape (Firecrawl)"),
    ...
    ("/v1/bounties/profile",  {"profile": "Test Expert Developer"}, True, "Bounty Profile"),
    ("/v1/bounties/action",   {"url": "http://test.com", "action": "bid"}, True, "Bounty Action"),
    ("/v1/infiltrator/stop",  {}, False, "Infiltrator Stop (safe)"),

    # ── Media / Video ──────────────────────────────────────────────
    ("/v1/video/cancel", {"job_id": "nonexistent-job-0001"}, False, "Video Cancel (non-existent)"),
    ("/v1/v2v/stop",     {}, False, "V2V Stop"),
    ("/v1/obs/stream/stop",  {}, False, "OBS Stream Stop (safe)"),
    ("/v1/obs/record/stop",  {}, False, "OBS Record Stop (safe)"),

    # ── HITL ───────────────────────────────────────────────────────
    ("/v1/hitl/approve", {"approval_id": "nonexistent-0001"}, False, "HITL Approve (non-existent)"),
    ("/v1/hitl/reject",  {"approval_id": "nonexistent-0001", "reason": "test"}, False, "HITL Reject (non-existent)"),

    # ── Gameserver ─────────────────────────────────────────────────
    ("/v1/gameserver/command", {"command": "list"}, False, "Gameserver Command (no server running)"),

    # ── Fabricaweb / Deploy ────────────────────────────────────────
    ("/v1/fabricaweb/deploy", {"url": "http://localhost:3000", "domain": "test.local"}, False, "Fabricaweb Deploy"),
]
...
def is_real_failure(status, body, path=""):
    ...
    if status == 0: return True   # Sin conexión — crash o timeout
    if status >= 500: return True  # Error interno del servidor
...
def check_body_ok(body):
    ...
    if isinstance(body, dict):
        if "ok" in body and body["ok"] is False: return False
        if "error" in body and not body.get("ok", True): return False
    return True
...
def run():
    ...
    proc = subprocess.Popen(["python", "bridge_server.py"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    with open("exhaustive_results_v2.json", "w", encoding="utf-8") as fout:
        json.dump({"passed": passed, "failed": failed, "total": total, "failures": failures}, fout, indent=2)
